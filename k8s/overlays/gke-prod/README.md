...
There is **no** `deploy-gke.yml` in this repository today (**removed** intentionally — see **`docs/FUTURE_WORK.md`**); production rollout from CI targets **EC2 + Compose** via **[`deploy-ec2.yml`](../../../.github/workflows/deploy-ec2.yml)** (triggered after successful service CI on `main`). To deploy these manifests from automation, **add your own workflow** (reuse image-build patterns from **`backend-ci`** / other CI workflows) pushing images to Artifact Registry/GHCR and `kubectl apply`/GitOps against your cluster. Typical inputs: **`--target production`** on the backend image; patch `image:` in this overlay or use a Kustomize `images:` transformer; GCP **Workload Identity** secrets where applicable.
