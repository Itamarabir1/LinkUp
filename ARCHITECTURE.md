...
- **Billing (Stripe):** דומיין `billing` תחת `POST /api/v1/billing/checkout`, `GET /api/v1/billing/status`, `GET /api/v1/billing/payments`, ו־`POST /api/v1/billing/webhook` עם אימות חתימה. מקור אמת חיוב: טבלת `payments`; `users.is_premium` נשמר כ-cache לקריאה מהירה.
