...
- **Avatar / Group images (S3 private)**: העלאה ישירה נשארת עם presigned PUT (`/users/me/avatar/upload-url`, `/groups/{id}/upload-image`), ותצוגה ללקוח מחזירה `avatar_url*` כ־**presigned GET קצר-תוקף** במקום URL ציבורי ישיר ל-bucket.
...
- **Domain-Driven Design**: כל דומיין (users, rides, bookings, passengers, chat, groups, **admin**, auth, …) — model, schema, crud, service; ראוטרים תחת `backend/app/domain/*/router.py` ונרשמים ב־[`api/v1/api_router.py`](backend/app/api/v1/api_router.py). **רישום מודלי SQLAlchemy:** בתחילת `api_router.py` מופיע `import app.db.models` (side-effect) כדי שכל הטבלאות יירשמו ב־`Base.metadata` לפני טעינת רואטרים (מניעת מעגליות / `"Group" failed to locate`). ב־[`alembic/env.py`](backend/alembic/env.py) אותו ייבוא לפני `target_metadata` ל־autogenerate. ב־Ruff: `per-file-ignores` ל־F401 על קבצי registry (`api_router.py`, `app/db/models.py`, `alembic/env.py`, `main_worker.py`) — ראו [`backend/pyproject.toml`](backend/pyproject.toml).
