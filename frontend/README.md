...
- **Frontend internal nginx:** קונטיינר הפרונט (`nginx.conf`, port 8080) מחזיר headers בטחוניים ברמת `server`: `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `Referrer-Policy: strict-origin-when-cross-origin`, `Permissions-Policy: camera=(), microphone=(), geolocation=(self)`. CSP/HSTS מכוונים מושמטים (מוגדרים ב-edge nginx בלבד).
...
- **Dockerfile secret hygiene** – Sentry secrets מועברים כ-`ARG` בלבד (ללא `ENV`) ב-builder stage. Docker ARG זמין אוטומטית כ-env var ל-`RUN npm run build` ולא נשמר ב-layer metadata — כך שגם `docker history` לא חושף את ה-token. ה-production stage (nginx) לא יורש כלום מה-builder מלבד `/app/dist`.
...
- **אימות** – קומפוננטות Login/Register/VerifyEmail מבוססות `react-hook-form + zod` עם שמירת behavior parity מול הזרימה הקיימת (אותם API calls, אותם נתיבי navigation ואותן הודעות שגיאה כלליות). ב-`Register` שדה `PhoneInput` מחובר דרך `Controller`. תמיכה ב-Google Sign-In באמצעות `GoogleSignIn.tsx` ו-`VITE_GOOGLE_CLIENT_ID`; GIS כ-module-level singleton (`gisLoader.ts`); fallback button מרונדר כ-React component (`<GoogleFallbackButton />`) במקום `innerHTML` — אפס DOM אימפרטיבי בזרימת auth (H20). בצד השרת יש **rate limiting** על רישום והתחברות (Redis) — בבדיקות עומס או ניסיונות חוזרים מהירים אפשר לקבל 429; ראו `docs/architecture/API.md` ו-`backend/README.md`.
...
- **Stage 5 cleanup — React Query + auth boot safety** – [`useMyRequests.ts`](src/pages/useMyRequests.ts) משתמש ב-`useInfiniteQuery`/`useMutation` מעל **`GET …/passenger/passengers/me`** בתגובה cursor pagination (**`PaginatedPassengerRequestsResponse`**: `items`, `next_cursor`, `has_more`; fetch עם `limit=100` ו-`cursor` לפי `getNextPageParam`), ו-cache patching ל־cancel/`REQUEST_EXPIRED` עובד על `InfiniteData.pages`. initial-load ב-[`AuthContext.tsx`](src/context/AuthContext.tsx) — `AbortController` ב-bootstrap `useEffect` (abort on unmount).
- **M2 — AbortController request cancellation (full-stack frontend):** כל ה-API layer הועבר לתמיכה ב-`signal` — פונקציות ב-[`api/chat.ts`](src/api/chat.ts), [`api/groups.ts`](src/api/groups.ts), [`api/users.ts`](src/api/users.ts), [`api/geo.ts`](src/api/geo.ts), [`api/presence.ts`](src/api/presence.ts), [`api/bookings.ts`](src/api/bookings.ts) מקבלות `opts?: { signal?: AbortSignal }` ומעבירות ל-Axios. **React Query migrations:** [`useGroupManageLists`](src/pages/GroupManage/useGroupManageLists.ts) (members + rides → `useQuery` עם `qk.groups.members` / `qk.groups.rides`), [`JoinGroup`](src/pages/JoinGroup.tsx) (invite lookup → `useQuery`), [`useMessageThread`](src/pages/MessageThread/useMessageThread.ts) (partner presence → `useQuery` עם `qk.presence.partner`), [`RouteMapModal`](src/components/RouteMapModal/index.tsx) (maps key → `useQuery` עם `qk.geo.mapsKey`). React Query v5 מעביר `signal` אוטומטית ל-`queryFn`. **Manual hooks:** [`useAbortSignal()`](src/hooks/useAbortSignal.ts) — utility hook שמחזיר `getSignal()`: מבטל signal קודם, יוצר חדש, ומבטל אוטומטית ב-unmount; מחליף את כל ה-`let cancelled = false` patterns. בשימוש ב-[`useConversationMessages`](src/pages/MessageThread/useConversationMessages.ts) (שתי instances: fetch + load-more), [`useChatPopup`](src/components/ChatPopup/useChatPopup.ts), ו-[`AuthContext`](src/context/AuthContext.tsx) bootstrap. בנוסף: [`useLocationBroadcast`](src/hooks/useLocationBroadcast.ts) (booking ID fetch → `AbortController` ישיר), [`useProfile`](src/pages/useProfile.ts) (avatar polling → `abortableDelay` + signal forwarding, abort on unmount). **שכבות שלא שונו:** WebSocket hooks (`ws.close()` מספיק), `useGoogleSignInScript` (script load), `useFCMCheck` (browser API), `fetchMissedGap` (`shouldAbort` callback מספיק). **Error handling:** `axios.isCancel(err)` ב-catch, ו-response interceptor ב-[`client.ts`](src/api/client.ts) כבר מסנן `ERR_CANCELED` מ-Sentry. פירוט: [`docs/ENGINEERING_HIGHLIGHTS.md`](../docs/ENGINEERING_HIGHLIGHTS.md), ADR Frontend §23.
...
- **שכבות WS שלא שונו במכוון במיגרציית Stage 3d** – [`useChatWebSocket`](src/context/useChatWebSocket.ts) ו-[`processChatWebSocketMessage`](src/pages/MessageThread/processChatWebSocketMessage.ts) נשארו transport/message-stream raw כדי לשמור יציבות בזמן מיגרציה מדורגת. [`useConversationMessages`](src/pages/MessageThread/useConversationMessages.ts) ו-[`useChatPopup`](src/components/ChatPopup/useChatPopup.ts) שודרגו עם `AbortController` (M2) אך נשארו מבוססי `useState` (לא RQ) בשל state מורכב + WS integration. **H2 — גלילה מבוססת כוונה:** `scrollIntentRef` (`'bottom'`/`'preserve'`/`'none'`) נקבע לפני כל `setMessages` — טעינה ראשונית/שליחה = bottom; טעינת היסטוריה = preserve (שימור offset); gap-fill = none; WS inbound = bottom רק אם קרוב לתחתית. `useChatPopup` — `shouldScrollRef` guard.
...
- **A11y — ConfirmModal aria fix (M12)** – הוסר `aria-hidden="true"` שגוי מ-backdrop wrapper ב-[`ConfirmModal.tsx`](src/components/ConfirmModal/ConfirmModal.tsx) שהסתיר את כל ה-dialog מ-screen readers; `aria-modal="true"` על ה-panel הפנימי מטפל נכון בסמנטיקת modal.
