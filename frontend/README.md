...
- **Zod + WebSocket** – סכימות משותפות ב־[`src/types/wsEvents.ts`](src/types/wsEvents.ts): אירועי נסיעה (`RideEventSchema`), מיקום נהג/נוסעים, אירועי צ’אט (`ChatPresenceEventSchema`). `safeParse` ב־`useRideWebSocket`, `useDriverLocation`, `usePassengerLocations`, `MyRides`, `processChatWebSocketMessage`. סיכום להצגה: [`docs/ENGINEERING_HIGHLIGHTS.md`](../docs/ENGINEERING_HIGHLIGHTS.md).
