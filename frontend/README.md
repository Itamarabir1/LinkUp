...
- **פיד התראות in-app (מסך / באדג’ צ’אט)** – רשימה מ־**`GET /api/v1/users/me/notifications`** ב־[`useChatNotificationsFeed.ts`](src/context/useChatNotificationsFeed.ts) (React Query + polling ~**5 דקות**). רענון חי: אירועי **`user:{id}:events`** על חיבור הצ’אט (`useUserEvent` / `useUserEventStream` ב־[`ChatContext.tsx`](src/context/ChatContext.tsx)), לא WS נפרד ב-FastAPI. **FCM (דחיפה):** [`services/fcm.ts`](src/services/fcm.ts) — לוגי דיבאג עיקריים ב־**`devLog`** רק ב־`import.meta.env.DEV`; פירוט: [`docs/FCM_SYSTEM_SUMMARY.md`](../docs/FCM_SYSTEM_SUMMARY.md).
