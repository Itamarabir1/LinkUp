...
| חיפוש נסיעות | `passengers` + `rides` + `geo` ב־`useSearchRides` (**`buildManualRideSearchParams`** לפי `searchMode`, **`buildParamsFromAiResult`** אחרי AI); join מחיפוש + idempotency ב־`useJoinRide.ts` | ✅ |
...
| התראות / unread | `users.fetchMyNotifications` + `chat.fetchUnreadMessageCount` ב־`ChatContext`; `useChatNotificationsFeed` (REST + polling ~5 דקות); רענון חי — `useUserEventStream` ב־`ChatContext` (`invalidate` + `UserEvent`, `setUnreadDirect` ל-unread); `useLayoutShell` לא מחזיק WS; ב־`Layout` — תגית + **`.iconBtnUnread`** כשיש ספירה | ✅ |
