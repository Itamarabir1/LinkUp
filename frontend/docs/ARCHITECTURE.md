...
- **Edge (production Compose):** enforcing **`Content-Security-Policy`** is set on **`nginx/nginx.conf`** (not in Vite); violations can be reported to Sentry. The build is a **static SPA** (no SSR), so future **nonce**-based CSP tightening needs edge HTML rewriting or hashes — see **[`docs/SECURITY_HEADERS.md`](../../docs/SECURITY_HEADERS.md)** and **[`docs/FEATURE_DECISIONS.md`](../../docs/FEATURE_DECISIONS.md#browser-csp-edge)**.
