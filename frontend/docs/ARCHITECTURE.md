...
| App shell | `src/components/Layout/` | Nav, outlet, global UI; shell logic in [`useLayoutShell.ts`](../src/components/Layout/useLayoutShell.ts) (profile menu, FCM init, chat popup visibility) |
...
| Shared hooks | `src/hooks/` | Reusable behavior (e.g. location broadcast) |
| Types | `src/types/` | Shared TS types (e.g. `Ride`) |
...
3. **Realtime**: WebSockets where needed (e.g. ride list updates in `MyRides`), separate from REST base URL (`config/env`).
