...
2. **הרצה עם Docker**
  ...
  - **Container resource limits:** כל השירותים מוגבלים ב-`mem_limit`, `mem_reservation`, `memswap_limit`, `cpus`, `pids_limit` דרך extension fields (`x-resources-micro/light/medium/redis`) בראש `docker-compose.yml`. תקציב מותאם ל-t3.medium (steady-state 3264MB מתוך ~3500MB usable). פרופיל monitoring מוסיף 768MB ודורש t3.large.
  ...
  - **פיתוח:** `docker compose up -d` → תשתית + **migrate** + 3 workers + backend (**8000**) + chat-ws (**8081**). פרונט: **`npm run dev`** בתיקיית `frontend`, לא קונטיינר. **FCM service worker:** `npm run dev` (ו-`vite build`) מייצרים אוטומטית `public/firebase-messaging-sw.js` מה-template דרך Vite plugin `firebaseSwPlugin` — אין צורך בצעד ידני; ודאו ש-`VITE_FIREBASE_*` מוגדרים ב-`frontend/.env`.
   ...
   - **סטאק מלא מאחורי Nginx (פורט 80):** לפני `docker compose --profile prod` הגדר **`SENTRY_REPORT_URI`** ב־`backend/.env` (endpoint של CSP reports מ־Sentry) והפעל **`bash scripts/ops/render-nginx-conf.sh`** כדי לייצר `nginx/nginx.conf` מה־template. אחר כך: `docker compose --profile prod up -d --build`. **Nginx** מפעיל **HTTP/2** (`http2 on`), **SSL/TLS hardening** via [`nginx/snippets/ssl-params.conf`](../../nginx/snippets/ssl-params.conf) (ECDHE-only AEAD ciphers, OCSP stapling, session cache, HSTS with `preload`), **gzip compression** (`gzip_proxied any` — קריטי כי כל התגובות עוברות proxy) ו-**API rate limiting** (`limit_req_zone` 30r/s per IP על `/api/v1/`, `burst=50 nodelay`, `limit_req_status 429`).
...
| SECRET_KEY | **כן (כל סביבה)** | ל-JWT; **חייב להיות non-empty** (crash ב-startup אם ריק); **≥32 תווים** נאכף ב-`ENVIRONMENT=production` |
| ALGORITHM | — | HS256 |
...
| BACKUP_S3_BUCKET | לגיבויים | שם S3 bucket לגיבויי DB (ברירת מחדל `linkup-db-backups`) |
| BACKUP_ENCRYPTION_KEY | לגיבויים | מפתח AES-256 להצפנת גיבויים; צור עם `openssl rand -base64 32` |
...
- **Cursor-based Pagination (כל ה-endpoints)**: נסיעות, הודעות, התראות, הזמנות (`/my-bookings`), היסטוריית נהג/נוסע — כולם cursor-based keyset pagination דרך `core/pagination/cursor.py`. Page-based offset נשאר רק ב-admin endpoints (שם צריך `total` ו-random page access).
...
- **למה refresh token hashed at rest (M4)**: `users.refresh_token` שומר SHA-256 hash — לא plaintext. SHA-256 נבחר כי הטוקנים הם JWTs עם אנטרופיה גבוהה (bcrypt מיותר). Hash ב-CRUD layer, verify ב-`hmac.compare_digest` (constant-time); generic `update` מוגן ב-**schema-derived allowlist** (`_allowed_update_fields = set(UserUpdate.model_fields.keys())`) — ברירת מחדל "חסום"; שדות רגישים (`is_admin`, `is_premium`, `is_active`, `is_verified`, `refresh_token`) דורשים CRUD methods ייעודיים (`mark_as_verified`, `update_refresh_token`). `UserUpdate` הופרד מ-`UserBaseSchema` (standalone `BaseModel` עם `extra="forbid"`); אותו `extra="forbid"` על `GroupUpdate` ו-`RideUpdate`. Migration **021** — null-ify קיימים.
