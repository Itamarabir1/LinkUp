...
- **Redis subscriber resilience (chat-ws):** all three Redis subscribers (`RunSubscriber` for chat/typing/user-events, `RunUserOfflineSubscriber`, `RunUserOnlineSubscriber`) wrap their subscribe logic in a **reconnect loop with exponential backoff** (1s initial, doubling to 30s cap). A transient Redis disconnect no longer kills the subscriber goroutine permanently — it logs a warning and reconnects automatically. **Backoff resets automatically** after a stable connection: if the inner subscribe function ran longer than `maxBackoff` (30s), the backoff resets to 1s on the next disconnect — prevents permanent 30s reconnection delay after months of accumulated disconnects. Implementation: `subscriber.go` extracts `runOnce`; `hub.go` extracts `runUserOfflineOnce`/`runUserOnlineOnce`.
...
- **Per-frame WebSocket writes (chat-ws C1 fix):** `RunWritePump` now sends each message as a **separate WebSocket text frame** via `Conn.WriteMessage(TextMessage, msg)` instead of batching multiple JSON objects into one frame with newline separators. The drain optimization (flushing queued messages from the `Send` channel in a tight loop) is preserved for syscall efficiency, but each message gets its own frame. Frontend consumers (`useChatWebSocket.ts`, `useUserEventStream.ts`) parse a single `JSON.parse(event.data)` per `onmessage` — no more `split('\n')` workaround. Backward-compatible: the frontend split was harmless on single-object frames during the rollout window.
...
- **אירועים per-user על אותו namespace (DB1)**: pattern **`user:*:events`** — מאחד:
  ...
  - לאחר **`send_message`** בצ'אט, **[`chat/router.py`](../../backend/app/domain/chat/router.py)** (ולא ה-service) מפרסם **`{ type: "invalidate", resource: "unread_messages", count }`** לעדכון באדג' unread בלי להמתין ל-polling. ה-count נשאב דרך **`SessionLocal`** טרי (session חדש אחרי commit) כדי לקבל ספירה מדויקת גם מאחורי PgBouncer.
...
- **Unread / פתיחת שיחה:** ב־[`chat/crud.py`](../../backend/app/domain/chat/crud.py) — `get_or_create_conversation` מוסיף שתי שורות ב־`conversation_participants` (אחת לכל משתמש); `mark_conversation_read` מעדכן `last_read_at` / `last_read_message_id` ואם אין שורה לקורא (נתוני legacy) יוצר אותה, כדי ש־`POST .../read` ו־`GET /chat/unread-count` יתאימו ל־`has_unread` באינבוקס.
