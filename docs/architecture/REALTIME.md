...
**Redis — שרת אחד, שני DB לוגיים:** אותו תהליך Redis (פורט 6379); **DB 0** — backend (cache, broadcast נסיעות `ride_*`, rate limit, OTP…); **DB 1** — צ'אט + **אירועי משתמש ל-chat-ws** (pub/sub הודעות, `user:{id}:events`, `presence:*`, `user:online` / `user:offline`, completion). **חשוב:** הזרימה הפעילה ל-completion היא Redis completion channel + `ai-worker` listener, לא Celery broker flow נפרד. הודעות צ'אט וגם **`publish_user_event`** מופעלים דרך **`REDIS_CHAT_URL`** / [`redis_chat_pubsub`](../../backend/app/infrastructure/redis/chat_pubsub.py) (ברירת מחדל DB **1**), כדי ש-**chat-ws** יקבל את ה-Pub/Sub. בפרודקשן, מקור הסיסמה של `chat-ws` מגיע מה-root compose env (`REDIS_PASSWORD`) דרך `REDIS_URL=redis://:${REDIS_PASSWORD}@redis:6379/1` ב־[`docker-compose.yml`](../../docker-compose.yml) — לא סיסמה hardcoded ב־`chat-ws/.env`. ערוצי נסיעה per-ride נשארים ב-**`broadcast`** על **`REDIS_URL`** (DB 0).
