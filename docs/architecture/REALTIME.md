...
- **פרונט:** [`useUserEventStream`](../../frontend/src/hooks/useUserEventStream.ts) על אותו חיבור chat-ws — קודם **`InvalidateEventSchema`**, אחר כך **`UserEventSchema`**; הלוגיקה ב-[`ChatContext.tsx`](../../frontend/src/context/ChatContext.tsx) (**`setUnreadDirect`** ל-unread מ-cache של React Query, **`refreshUnreadNotifications`** + **`linkup:user-event`** לתאימות עם מסכים שמקשיבים ל-custom event). **`useLayoutShell`** לא מחזיק עוד מאזין WS.
