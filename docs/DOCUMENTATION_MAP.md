...
- **Compose — שירותים ופרופילים:** [`docker-compose.yml`](../docker-compose.yml) (prod, monitoring).
- **CI backend — סדר שלבים:** [`.github/workflows/backend-ci.yml`](../.github/workflows/backend-ci.yml) (Ruff → Alembic → `check-migration-head` → pytest RabbitMQ נקודתי → pytest מלא; push ל־`main` בונה תמונות `linkup/backend`, `worker`, `migrate`, `pgbouncer`). **פריסת EC2** — [`.github/workflows/deploy-ec2.yml`](../.github/workflows/deploy-ec2.yml) (`workflow_run` אחרי אחד מ־Backend CI / Frontend CI / Chat-WS CI / Email renderer CI על `main`): `envsubst` ל־**`nginx/nginx.conf`** מ־**`SENTRY_REPORT_URI`** ב־`backend/.env`, smoke פנימי ל־`frontend` + `email-renderer` לפני gate ל־`backend`, rollback עם fallback ל־**`backend:latest`**.
...
| איזה hook בפרונט יושב על איזה WS | [`frontend/src/hooks/`](../frontend/src/hooks/) (`useUserEventStream`, `useRideWebSocket`, `useReconnectingWebSocketState`, …) מול [`docs/ENGINEERING_HIGHLIGHTS.md`](ENGINEERING_HIGHLIGHTS.md) |(prod, monitoring). מפת תיעוד (Documentation map)
