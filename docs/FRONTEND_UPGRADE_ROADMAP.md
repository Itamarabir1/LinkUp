...
- [~] Post-deploy verification: smoke login/chat/maps/uploads/billing; watch Sentry CSP reports; tune allowlists if needed (edge truth: **`nginx/nginx.conf.template`** + **`scripts/ops/render-nginx-conf.sh`**).
