...
| **עדכון S.7** | `common`/`nav`/`admin` נשארים **bundled** ב-`src/i18n/config.ts`, בעוד namespaces פיצ'ריים (`auth`,`rides`,`bookings`,`groups`,`profile`,`billing`) נטענים עצלנית דרך `i18next-http-backend` מ-`/locales/{{lng}}/{{ns}}.json` (מקור אמת נשאר ב-`src/i18n/locales`, והעתקה runtime ב-`public/locales`). |
| **Admin i18n** | מסכי אדמין (`/admin/*`) עברו ל-i18n מלא — namespace **`admin`** (bundled) עם ~100 מפתחות; כל דף משתמש ב-`useTranslation('admin')` ואפס מחרוזות עברית קשיחות בקומפוננטות. `adminConstants.ts` (labels לגרפים) נשאר as-is — hooks לא ניתנים לשימוש בקובצי constants; מפתחות מקבילים (`ride_status_*`, `booking_status_*`) קיימים ב-namespace לשימוש עתידי. |
