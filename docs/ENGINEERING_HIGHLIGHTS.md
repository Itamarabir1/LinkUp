...
- **Stage 2 hardening batch (security + resilience + logging hygiene):**
  ...
  - **Logging hygiene (H5):** הוסרו קריאות `print()` מ-`OutboxRepository.save_event` ומ-`register` endpoint ב-`router.py` — `logger.info` כבר מכסה את אותו מידע; `print` לא משתלב עם מערכת הלוגים המובנית (JSON/structured).
