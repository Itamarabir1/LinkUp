...
| **i18n / טיפוגרפיה** | **`LangContext`** — `lang`, `dir`, **`--font-primary`**; קבצי תרגום תחת `src/i18n/locales/` (8 namespaces, ~375 מפתחות פעילים אחרי ניקוי 57 מפתחות מתים); ב־**`*.module.css`** — `var(--font-primary)` / `var(--font-numeric)` (חריג: `LangToggle`). UI copy ב-`Messages.tsx` דרך **`common:msg_*`** / **`common:unread_count`** / **`common:user_fallback`**. |
