...
- **Frontend-only CD to EC2 (`workflow_run`):** נוסף **[`.github/workflows/deploy-frontend-ec2.yml`](../.github/workflows/deploy-frontend-ec2.yml)** — רץ אחרי ש־**Frontend CI** על **`main`** מסתיים ב־**success**; SSH ל־EC2 (אותם **`EC2_*`** secrets כמו ב־backend deploy), `git` sync, `envsubst` ל־**`nginx/nginx.conf`**, `docker pull …/frontend:latest`, **`compose --profile prod up -d --no-deps frontend`** ואז **`--force-recreate nginx`**, smoke פנימי: **`docker exec linkup_frontend wget`** ל־**`http://localhost:80/config.js`** (לא תלוי ב־DNS/HTTPS מה-runner). מבטיח פריסת UI בפרודקשן בלי תלות ב־path filters של **backend-ci**. פירוט: **[`docs/DEPLOYMENT.md`](DEPLOYMENT.md)** (CI/CD), **[`README.md`](../README.md)** (טבלת CI/CD).
...
| מה | פירוט |
|----|--------|
| **GitHub Actions — workflows עם path filters** | **`backend-ci`**: Ruff (+ format check), Postgres שירות, **`DATABASE_URL=test_db`**, **`uv sync`**, **`uv run alembic upgrade head`**, **`scripts/ops/check-migration-head.sh`**, **`tests/infrastructure/test_rabbitmq_reliability.py`**, **`uv run pytest tests/`**. **`frontend-ci`**, **`chat-ws-ci`**, **`email-renderer-ci`** — טריגר לפי `paths` בקבצי ה-workflow; פריסת **production** מה־**`backend-ci`** על **EC2** (SSH + smoke), לא מ־GKE workflow (מניפסטי **`k8s/`** זמינים אך workflow GKE לא במאגר — **`docs/FUTURE_WORK.md`**). |
