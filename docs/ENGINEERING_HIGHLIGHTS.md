...
| **פיד התראות (in-app)** | **`useChatNotificationsFeed`** — REST **`GET /users/me/notifications`** + polling **~5 דקות**; רענון חי דרך **`useUserEvent` / `useUserEventStream`** על **`user:{id}:events`** (chat-ws); משולב ב־`ChatContext`. |
