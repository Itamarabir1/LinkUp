...
- **Stage 2 hardening batch (security + resilience + logging hygiene):**
  ...
  - **`model_validator` ל-SECRET_KEY (H1):** `config.py` משתמש ב-Pydantic v2 `model_validator(mode="after")` (לא `field_validator`) כדי לאכוף **≥32 תווים** כש-`ENVIRONMENT=production`. מריצה אחרי שכל השדות נטענו — אמין לגישת `self.ENVIRONMENT` ו-`self.SECRET_KEY`.
...
| **Refresh token storage (M4)** | **SHA-256 hash** ב-DB — plaintext לא נשמר אף פעם. Hash ב-CRUD (`update_refresh_token`); השוואה ב-`verify_refresh_token` עם **`hmac.compare_digest`** (constant-time); `refresh_token` ב-`protected_fields` של generic `update` נגד bypass; migration **021** — null-ify קיימים. |
