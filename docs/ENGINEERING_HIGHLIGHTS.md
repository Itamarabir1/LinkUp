...
| **שגיאות** | `getApiErrorMessage` / `getApiStatus` / `isTimeoutOrAbortError` ב־`utils/apiError.ts` + **Vitest** (`apiError.test.ts`). ב־hooks: fallback אחיד עם **`apiErr('err_*')`** (מפתחות ב־`common.json`) במקום מחרוזות עברית קשיחות. **`isTimeoutOrAbortError`** בשימוש אחיד גם ב-`Login.tsx` וגם ב-`useGoogleSignIn.ts` (H20 refactor). |
