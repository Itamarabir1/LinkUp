...
- **Production bug fixes (H2, H6, H7-backoff, H8-types):**
  - **H2 — Chat scroll-to-bottom on history load (frontend UX):** `useEffect(() => scrollIntoView(), [messages])` in [`useConversationMessages.ts`](../frontend/src/pages/MessageThread/useConversationMessages.ts) and [`useChatPopup.ts`](../frontend/src/components/ChatPopup/useChatPopup.ts) fired on **every** `messages` state change — including older-history prepend (`loadMoreMessages`), reconnect gap-fill (`fetchMissedMessages`), and WS inbound. Replaced with **intent-based scrolling**: a `scrollIntentRef` (`'bottom'` / `'preserve'` / `'none'`) set before each `setMessages` call, consumed in a `useLayoutEffect`. Initial load and own sends force scroll-to-bottom; `loadMoreMessages` preserves scroll offset (`scrollTop += scrollHeight delta`); gap-fill and inbound WS messages scroll only if the user is already near the bottom (`isNearBottom` threshold 150px). `scrollerRef` added to [`MessageThreadPanel.tsx`](../frontend/src/pages/MessageThread/MessageThreadPanel.tsx) container div. `useChatPopup` uses a simpler `shouldScrollRef` guard (no load-more path).
  - **H6 — `http.Client` per-iteration resource leak (chat-ws):** `flushDueLastSeen` in [`hub.go`](../chat-ws/internal/hub/hub.go) created a new `&http.Client{}` for **every key** in a Redis SCAN loop running every 5s — each disposable client allocated its own `http.Transport` and TCP connection pool, leaking file descriptors under load. `fetchLastSeenFromBackend` in [`presence.go`](../chat-ws/internal/api/presence.go) had the same pattern per HTTP request. Fix: added `httpClient *http.Client` field to `Hub` struct (initialized in `NewHub` with tuned `Transport`: `MaxIdleConns: 20`, `MaxIdleConnsPerHost: 10`, `IdleConnTimeout: 30s`); `HandlePresence` closure captures a shared `backendClient` created once at startup. Both sites now reuse a single connection pool.
  - **H7 — Redis subscriber backoff never resets (chat-ws):** all three subscriber loops (`RunSubscriber`, `RunUserOfflineSubscriber`, `RunUserOnlineSubscriber`) used exponential backoff (1s→30s cap) that only ever **increased** — after enough disconnects over weeks/months, reconnection permanently slept the full 30s. Fix: track `time.Since(start)` after each `runOnce`/`runUser*Once` call; if the inner function ran longer than `maxBackoff` (indicating a stable connection, not a crash loop), reset `backoff` to `time.Second`. Frontend WS hooks were already correct (`attempt = 0` on `onopen`).
  - **H8 — `user_id: int` type mismatch in backend (remnant of pre-UUID migration):** DB PK is `PG_UUID(as_uuid=True)` but 4 methods in [`users/service.py`](../backend/app/domain/users/service.py) annotated `user_id` as `int`; also mismatched in [`exceptions/user.py`](../backend/app/core/exceptions/user.py) (`UserNotFoundError`), [`calendar_export.py`](../backend/app/domain/chat/calendar_export.py) (`conversation_id` + `current_user_id`), and [`s3/service.py`](../backend/app/infrastructure/s3/service.py) (vestigial `int` in union). All corrected to `UUID`; dead commented-out route with `int` type in `users/router.py` removed. Go and TypeScript layers were already correct (`string` throughout).

- **Audit fixes (M5, M7, M3):**
  - **M5 — Redis password hidden from process list:** `docker-compose.yml` redis service previously passed `--requirepass "$REDIS_PASSWORD"` as a command-line argument, exposing the secret in `/proc/PID/cmdline` (`ps aux`, `docker top`). Replaced with a runtime config file approach: `printf 'requirepass %s\n' "$REDIS_PASSWORD" > /tmp/redis.conf && exec redis-server /tmp/redis.conf ...` — process list shows `redis-server /tmp/redis.conf`, not the password. Healthcheck migrated from `redis-cli -a` to `REDISCLI_AUTH` env var (not visible in process list). File: [`docker-compose.yml`](../docker-compose.yml).
  - **M7 — Password reset/change invalidates sessions:** `change_password` and `reset_password_with_code` in [`auth/service.py`](../backend/app/domain/auth/service.py) previously only updated the password hash without revoking existing sessions. An attacker with a valid refresh token could continue refreshing indefinitely after the victim resets their password. Fix: added `_invalidate_sessions` helper that clears `user.refresh_token` in DB (same transaction as password update). Next refresh attempt by any device returns 401 — forces re-login. Existing access tokens expire naturally within 30 min (acceptable tradeoff vs. tracking all `jti`s per user).
  - **M3 — TOCTOU race in group join (`SELECT FOR UPDATE`):** `join_by_invite` in [`groups/service.py`](../backend/app/domain/groups/service.py) had a check-then-act race on `max_members`: concurrent requests each saw the committed count before the other's INSERT, allowing overfill. Fix: `get_group_for_update` in [`groups/crud.py`](../backend/app/domain/groups/crud.py) acquires `SELECT ... FOR UPDATE` on the group row, serializing concurrent joins. Additionally, `IntegrityError` from the `uq_group_member` constraint is now caught and returned as a clean 409 (was unhandled 500).

- **Critical audit fixes (C1–C4):**
  - **C3 — Google OAuth outbox atomicity:** `authenticate_with_google` previously had two separate `db.commit()` calls — user creation committed first, then the `user.registered` outbox event in a second commit. A crash between the two lost the event permanently. Restructured to a single atomic commit: user + Google link + outbox event all flush in one transaction, matching the `register_new_user` pattern. File: [`backend/app/domain/auth/service.py`](../backend/app/domain/auth/service.py).
  - **C1 — WebSocket per-frame writes (chat-ws + frontend):** `RunWritePump` in [`conn.go`](../chat-ws/internal/hub/conn.go) previously batched multiple JSON messages into a single WebSocket text frame separated by `\n` (gorilla chat example pattern). Rewritten to send each message as its own `WriteMessage(TextMessage, ...)` call — proper WebSocket framing for independent JSON objects. Frontend consumers (`useChatWebSocket.ts`, `useUserEventStream.ts`) no longer need `split('\n')` — each `onmessage` event is a single `JSON.parse(event.data)`.
  - **C2 — `test-push` endpoint guarded:** `POST /users/me/test-push` was accessible to all authenticated users in production (bypasses dedup/rate-limit/NotificationManager). Now returns 404 when `settings.DEBUG` is `False` — endpoint invisible in production. File: [`backend/app/domain/users/router.py`](../backend/app/domain/users/router.py).
  - **C4 — `_safe_send` contract fulfilled:** `NotificationManager._safe_send` docstring claimed it returns exceptions on failure, but had no `try/except`. Now genuinely catches exceptions: known `PermanentNotificationError`/`TransientNotificationError` re-raised; unknown exceptions wrapped as `TransientNotificationError` (safe default: retry rather than drop). `asyncio.gather(return_exceptions=True)` remains as defense-in-depth. File: [`backend/app/domain/notifications/manager.py`](../backend/app/domain/notifications/manager.py).

- **Groups CRUD flush-only + update schemas hardened (architecture-level):**
  - **Transaction ownership extended to groups:** all 8 write methods in `groups/crud.py` (`create_group`, `join_group`, `remove_member`, `rename_group`, `update_group_description`, `update_group_avatar_key`, `close_group`, `update_member_role`) refactored from `db.commit()` to `db.flush()`. Callers (service/router) now own the transaction boundary. Multi-step mutations like `update_group` (rename + description) are now **atomic**. Consistent with the users CRUD pattern (ADR §29).
  - **Protected fields allowlist — defense in depth:** `UserUpdate` detached from `UserBaseSchema` (no longer inherits `password`/`new_password`); standalone `BaseModel` with only `full_name`, `email`, `phone_number`. `model_config = ConfigDict(extra="forbid")` added to `UserUpdate`, `GroupUpdate`, and `RideUpdate` — Pydantic v2 now rejects unknown fields with 422. Three-layer defense: (1) schema `extra="forbid"`, (2) CRUD allowlist (`_allowed_update_fields`), (3) DB constraints.
  - **`/my-bookings` migrated to cursor pagination:** `GET /bookings/my-bookings` moved from page-based offset (`page`/`total`) to cursor-based keyset pagination (`after`/`next_cursor`) on `(created_at DESC, booking_id DESC)` via the shared `core/pagination/cursor.py` utility. Now consistent with all other paginated endpoints (driver/passenger history, notifications, rides, chat). Frontend `fetchMyBookings` updated to use `after`/`next_cursor` instead of `page`/`offset`.

- **M2 — AbortController request cancellation (frontend, architecture-level):** ריפקטור מלא של שכבת הביטול בפרונט — מאפס שימושים ב-`AbortController` ל-coverage מלא ב-14 useEffect hooks ו-10 קבצים. **שכבה 1 (API):** כל פונקציות ה-API הידניות (`chat.ts`, `groups.ts`, `users.ts`, `geo.ts`, `presence.ts`, `bookings.ts`) מקבלות `opts?: { signal?: AbortSignal }` ומעבירות ל-Axios; generated client (Orval) כבר תומך ב-signal דרך `apiMutator`. **שכבה 2 (utility hook):** [`useAbortSignal()`](../frontend/src/hooks/useAbortSignal.ts) מחזיר `getSignal()` שמבטל signal קודם, יוצר חדש, ומבטל אוטומטית ב-unmount — מחליף כל `let cancelled = false` pattern. **שכבה 3 (React Query migrations):** `useGroupManageLists` (members + rides → `useQuery` עם `qk.groups.members`/`qk.groups.rides`; `loadMembers`/`loadRides` → `invalidateQueries`), `JoinGroup` (invite lookup → `useQuery`), `useMessageThread` (partner presence → `useQuery` + `qk.presence.partner`, WS continues via local state), `RouteMapModal` (maps key → `useQuery` + `qk.geo.mapsKey`, `staleTime: Infinity`). React Query v5 מעביר signal אוטומטית. **שכבה 4 (manual AbortController):** `useConversationMessages` (שתי instances של `useAbortSignal` — fetch + load-more, `axios.isCancel` ב-catch), `useChatPopup` (אותו pattern), `AuthContext` bootstrap (`new AbortController` ב-useEffect + cleanup), `useLocationBroadcast` (booking ID fetch → `AbortController` ישיר), `useProfile` avatar polling (`abortableDelay` + signal forwarding + unmount abort). **לא שונו (ומדוע):** WebSocket hooks (`ws.close()` מספיק), `useGoogleSignInScript` (script load), `useFCMCheck` (browser API), `fetchMissedGap` (`shouldAbort` callback מספיק — שני מנגנוני ביטול מקבילים מיותרים). Error handling: response interceptor כבר מסנן `ERR_CANCELED` מ-Sentry. ADR Frontend §23.

- **Automated PostgreSQL backup pipeline:** multi-layer backup strategy for the self-hosted PostgreSQL on EC2. Docker Compose `db-backup` service (profile `backup`) builds a container with `postgres:15` + AWS CLI + `openssl`; runs `pg_dump --format=custom --compress=6` → AES-256-CBC encryption → S3 upload with verification. Two triggers: **daily cron** (03:00 UTC via host crontab) and **pre-deploy** (non-blocking step in `deploy-ec2.yml` before image pulls). S3 lifecycle: daily backups Glacier at 14d, expire at 30d; pre-deploy Glacier at 30d, expire at 90d. Restore script (`restore.sh`) supports latest-or-specific S3 key retrieval with post-restore row-count sanity check. Monitoring: `scripts/ops/check-backup-health.sh` verifies freshness (<25h) and minimum size. Full disaster recovery runbook (including full EC2 rebuild) in [`BACKUP_AND_RECOVERY.md`](BACKUP_AND_RECOVERY.md). Phase 2 roadmap: RDS migration with managed PITR. **[`docs/BACKUP_AND_RECOVERY.md`](BACKUP_AND_RECOVERY.md)**, **[`docs/FEATURE_DECISIONS.md`](FEATURE_DECISIONS.md#db-backup-strategy)**, **ADR backend §30**.

- **Server-side notification read-state (full-stack, מיגרציה 023):** מצב קריאת התראות הועבר מ-`localStorage` (שצבר נתונים ללא הגבלה, לא סונכרן בין מכשירים, לא היה מוגבל למשתמש) לטבלת **`notification_reads`** בצד השרת. PK מורכב `(user_id, booking_id, created_at)` תואם למפתח הזהות הקיים. **בקאנד:** מודל **`NotificationRead`**, **`PATCH /users/me/notifications/read`** (idempotent upsert לפריטים ספציפיים) ו-**`PATCH …/read-all`** (batch); `GET …/notifications` מחזיר `is_read` לכל פריט ו-`unread_count` בעמוד; batch-fetch read-state ב-`_get_read_keys` (שאילתת `IN` על booking IDs בלבד). **בפרונט:** `chatNotificationStorage.ts` נמחק; `useChatNotificationsFeed` קורא PATCH API + invalidate RQ; `chatState` reducer מקבל `unreadCount` מהשרת במקום לספור מול localStorage; `Notifications.tsx` קורא `n.is_read` ישירות; מיגרציה חד-פעמית ב-`ChatProvider` שולחת bulk POST של מפתחות ישנים ומוחקת את המפתח; `clearTokens` מנקה legacy key ב-logout. **[`docs/architecture/DATABASE.md`](architecture/DATABASE.md)**, **[`docs/architecture/API.md`](architecture/API.md)**, **[`docs/architecture/NOTIFICATIONS.md`](architecture/NOTIFICATIONS.md)**.

- **WebSocket token freshness gate + visibility-aware reconnect (frontend):** Access token (30 min TTL) נמסר כ-`?token=` ב-URL של כל WebSocket. כשלשונית רצה ברקע הדפדפן מצמצם timers — ה-pong לא מגיע בזמן (60s deadline בצד Go) והשרת סוגר חיבור; reconnect loop קרא token ישן מ-`localStorage` בלי לבדוק `exp` → לולאת כשלון אינסופית. **תיקון ברמת ארכיטקטורה:** (1) חילוץ לוגיקת refresh לקובץ משותף [`tokenRefresh.ts`](../frontend/src/api/tokenRefresh.ts) עם **`ensureFreshToken()`** — פענוח `exp` client-side ([`tokenUtils.ts`](../frontend/src/utils/tokenUtils.ts), `atob` בלבד, ללא crypto); אם < 60s לתפוגה → `POST /auth/refresh` (HttpOnly cookie) עם coordinated single-flight queue שמונע מרוץ refresh כשכמה WS hooks וגם Axios interceptor רצים במקביל. (2) **`visibilitychange`** listener בכל hook (`useReconnectingWebSocket`, `useReconnectingWebSocketState`, `useChatWebSocket`): בחזרה מרקע — אם אין WS פתוח, reconnect מיידי (attempt=0) עם token טרי. (3) Axios 401 interceptor ב-[`client.ts`](../frontend/src/api/client.ts) מואצל לאותו **`coordinatedRefresh()`** — מקור אמת יחיד לרענון token. **`FEATURE_DECISIONS.md`** [`#frontend-ws-token-freshness`](FEATURE_DECISIONS.md#frontend-ws-token-freshness).
...
- **Infrastructure refactor (senior-grade Docker/Compose/CI hardening):**
  ...
  - **Container resource limits (tiered profiles):** all services are constrained via YAML extension fields (`x-resources-micro`, `x-resources-light`, `x-resources-medium`, `x-resources-redis`) at the top of `docker-compose.yml`. Each profile bundles `mem_limit`, `mem_reservation`, `memswap_limit` (set equal to `mem_limit` — disables swap entirely), `cpus`, and `pids_limit`. Budget is sized for t3.medium (steady-state 3264MB / ~3500MB usable). Monitoring profile (prometheus + grafana) adds 768MB — requires t3.large or temporary use only.
  ...
  - **chat-ws healthcheck:** `docker-compose.yml` `wget` healthcheck references `/healthz`; the Go handler is now implemented (see chat-ws operational hardening below).

- **chat-ws operational hardening (H7–H11):**
  - **H7 — `/healthz` with subscriber liveness:** handler at [`cmd/server/main.go`](../chat-ws/cmd/server/main.go) checks Redis PING **and** `Hub.SubscribersHealthy()` — three `atomic.Int64` timestamps (chat/offline/online subscribers), updated on each successful `PSubscribe`/`Subscribe`, checked against a 2-minute staleness threshold. Seeded to `now` in `NewHub` for startup grace window. Returns `503 + {"detail":"subscriber stale"}` when a subscriber goroutine is stuck in reconnect backoff — catches the exact failure mode where chat-ws appears alive but delivers zero messages.
  - **H8 — Graceful HTTP shutdown:** replaced raw `http.ListenAndServe` with `*http.Server` + `srv.Shutdown(shutdownCtx)` (10s timeout). On SIGTERM: `cancel()` stops Redis subscribers, then `Shutdown` drains active WS connections gracefully instead of abrupt kill.
  - **H9 — Pong/read deadline (dead client detection):** [`handler.go`](../chat-ws/internal/hub/handler.go) sets `conn.SetReadDeadline(pongWait)` + `conn.SetPongHandler(...)` before the read loop. The write pump already sends pings every ~54s; if a client fails to reply within 60s, `ReadMessage()` returns a deadline error and the connection is cleaned up. Dead clients no longer leak goroutines and hub entries indefinitely.
  - **H10 — Panic recovery in all goroutines:** shared [`internal/safego/safego.go`](../chat-ws/internal/safego/safego.go) (`RecoverPanic` — logs panic value + full stack trace via `slog` + `runtime/debug`). Added `defer safego.RecoverPanic(...)` to every independent goroutine: `RunWritePump` ([`conn.go`](../chat-ws/internal/hub/conn.go)), `RunSubscriber` ([`redis/subscriber.go`](../chat-ws/internal/redis/subscriber.go)), `RunUserOfflineSubscriber`, `RunUserOnlineSubscriber`, `RunLastSeenDebounceWorker` ([`hub.go`](../chat-ws/internal/hub/hub.go)). A single goroutine panic no longer brings down the entire process.
  - **M6 — WebSocket origin validation hardened:** `CheckOrigin` in [`handler.go`](../chat-ws/internal/hub/handler.go) rewritten: pre-computed `map[string]struct{}` allowlist (O(1) lookup), empty `Origin` header now **rejected** when `ALLOWED_ORIGINS` is configured (previously returned `true` — allowed bypass), case-insensitive + trailing-slash-tolerant normalization via `strings.ToLower`/`TrimRight`, and `slog.Warn` structured logging on rejected origins for operational visibility.
  - **H11 — `sync.Once`-guarded connection teardown:** `Conn.done` channel was previously closed directly by the handler `defer` block, but concurrent hub broadcasts could race with teardown (a second `close(c.done)` panics in Go). Replaced with `Conn.Close()` (`sync.Once` guarantees single close) and `Conn.Done()` (read-only `<-chan struct{}` accessor). All call sites in [`hub.go`](../chat-ws/internal/hub/hub.go), [`handler.go`](../chat-ws/internal/hub/handler.go), and [`conn.go`](../chat-ws/internal/hub/conn.go) migrated — double-close panics are now **structurally impossible**.
  - **Architecture note:** `safego` is a shared `internal/` package (not duplicated per package). Subscriber liveness uses `atomic.Int64` — zero-contention on the hot path, no mutex needed.

- **Stage 5 hardening (production safety, performance, CI speed):**
  - **Production DEBUG guard:** `backend/app/main.py` now raises `RuntimeError` at import time if `ENVIRONMENT=production` and `DEBUG=True` — process crashes before serving any request, preventing accidental debug mode in production.
  - **Nginx gzip compression:** `nginx/nginx.conf.template` enables `gzip on` with `gzip_proxied any` (critical for reverse-proxy architectures), `gzip_min_length 1024`, and a curated `gzip_types` list covering JSON/JS/CSS/XML/SVG. Reduces response sizes ~60-80% for text payloads.
  - **Nginx SSL/TLS hardening:** TLS tuning extracted to [`nginx/snippets/ssl-params.conf`](../nginx/snippets/ssl-params.conf) (included in server block) — ECDHE-only AEAD cipher suite (no CBC/3DES/RC4/DHE), `ssl_prefer_server_ciphers on`, `ssl_session_cache shared:SSL:10m` + `ssl_session_tickets off` (forward secrecy), OCSP stapling (`ssl_stapling on` + `ssl_stapling_verify on` with `chain.pem`), `ssl_ecdh_curve X25519:secp384r1:secp256r1`. HTTP/2 enabled (`http2 on`). HSTS upgraded to include `preload`. Port 80 ACME challenge location for zero-downtime certbot renewal.
  - **Nginx API rate limiting:** `limit_req_zone` at 30r/s per IP (`$binary_remote_addr`, 10MB zone ~160K IPs) applied to `/api/v1/` with `burst=50 nodelay`. Returns `429 Too Many Requests` (not default 503) via `limit_req_status 429` — semantically correct for API clients with retry-after logic.
  - **Grafana password parameterized:** `GF_SECURITY_ADMIN_PASSWORD` in docker-compose.yml now uses `${GRAFANA_ADMIN_PASSWORD:-admin}` (compose variable substitution with dev-friendly default). Documented in `backend/.env.example`.
  - **Deploy prune safety:** `docker image prune -af` changed to `docker image prune -f` in `deploy-ec2.yml` — preserves tagged images as local cache for faster pulls and reliable rollback (the `-a` flag was deleting rollback-eligible images before re-pulling).
  - **CI Docker layer cache (GHA):** all three CI workflows (`backend-ci`, `frontend-ci`, `chat-ws-ci`) now use `docker/setup-buildx-action@v3` + `cache-from: type=gha` / `cache-to: type=gha,mode=max` on every `docker/build-push-action` step. `actions: write` permission added for cache writes. Expected build time reduction: 3-5 min → 30-60s on cache hits.
  - **Frontend console.log removed:** removed `console.log('[LinkUp Frontend] API Base URL:', ...)` from `frontend/src/api/client.ts` — information disclosure cleanup (leaks internal API base URL to browser console).
...
- **Production hardening checklist (completed):**
  ...
  - deploy disk pressure mitigated with safe image prune strategy (`-f` dangling only, not `-af`)
  ...
  - Edge **enforcing CSP** on Compose: **`nginx/nginx.conf.template`** → rendered **`nginx/nginx.conf`** ([**`scripts/ops/render-nginx-conf.sh`**](../scripts/ops/render-nginx-conf.sh) locally; CI `envsubst '${SENTRY_REPORT_URI}'` on EC2; **`SENTRY_REPORT_URI`** in **`backend/.env`**). **`Content-Security-Policy`** (was Report-Only) with curated **`script-src`** ללא **`'unsafe-inline'`** (קובץ **`public/bootstrap.js`** [`frontend/public/bootstrap.js`](../frontend/public/bootstrap.js) כ־**`/bootstrap.js`** לפני **`/config.js`**); **`connect-src`** / **`img-src`** for Firebase, Sentry ingest, GA/GTM, maps, uploads, **`accounts.google.com`** (GIS SDK); **`frame-src`** includes Stripe + **`accounts.google.com`** (GIS); **`style-src`** עדיין עם **`'unsafe-inline'`** ל-Vite/CSS; **`report-uri`** from env (no hardcoded ingestion URL in Git); CSP complements app/API XSS controls — see **`docs/SECURITY_HEADERS.md`**.

- **CI supply chain hardening (SHA-pinned GitHub Actions):** all **27** action references across **6** workflow files (`backend-ci`, `frontend-ci`, `chat-ws-ci`, `email-renderer-ci`, `openapi-contract`, `deploy-ec2`) are pinned to **immutable commit SHAs** instead of mutable tags (`uses: owner/action@<sha> # vX`). This prevents a compromised or moved tag from silently running unreviewed code in CI. Original version tags are preserved as trailing comments for readability. Dependabot `github-actions` ecosystem (weekly) opens automated PRs when new action versions are available, keeping the SHAs current without manual lookup.

- **Production hardening audit (senior-grade):**
  - **Container log rotation:** shared `x-default-logging` YAML anchor in `docker-compose.yml` (`json-file` driver, `max-size: 10m`, `max-file: 3`) applied to all 16 services. Prevents disk exhaustion on long-running EC2 — caps each container at 30MB of retained logs.
  - **Monitoring port hardening:** Prometheus (`9090`) and Grafana (`3002`) ports bound to `127.0.0.1` in Compose — not reachable from the public internet, accessible only from the host or via SSH tunnel.
  - **Frontend internal nginx security headers:** `frontend/nginx.conf` now returns `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `Referrer-Policy: strict-origin-when-cross-origin`, `Permissions-Policy: camera=(), microphone=(), geolocation=(self)` at `server` level — defense-in-depth even when the outer Compose nginx is bypassed. CSP/HSTS intentionally omitted (set by edge nginx only to avoid double-header conflicts). See **[`docs/SECURITY_HEADERS.md`](SECURITY_HEADERS.md)**.
  - **Database check constraints (migration 024):** `CHECK` constraints on `rides` (`available_seats >= 1`, `price >= 0`, `distance_km >= 0`, `duration_min >= 0`) and `bookings` (`num_seats >= 1`) — business invariants enforced at the PostgreSQL level regardless of application-layer bugs. See **[`docs/architecture/DATABASE.md`](architecture/DATABASE.md)**.
  - **Pagination max enforcement (audit-verified):** every paginated endpoint across all routers uses FastAPI `Query(le=...)` — public endpoints capped at 50–100, admin endpoints at 200–500. Constants (`_QUERY_MAX_USERS_LIMIT`, `_QUERY_MAX_ADMIN_LIST_LIMIT`) defined once at module top in `admin/router.py`.
...
- **Supply chain baseline (Dependabot):** `.github/dependabot.yml` opens scheduled dependency PRs for:
  ...
  - **github-actions** (`/`, weekly) — keeps SHA-pinned action references up to date (labels: `dependencies`, `ci`).
...
- **Centralized EC2 deploy (`workflow_run`):** **[`.github/workflows/deploy-ec2.yml`](../.github/workflows/deploy-ec2.yml)** — אחרי **Backend CI** / **Frontend CI** / **Chat-WS CI** / **Email renderer CI** על **`main`** ב־**success**; SSH (אותם **`EC2_*`**), compose מלא, smoke פנימי ל־**`linkup_frontend`** (**`config.js`**) ול־**`linkup_email_renderer`** (**`/health`**) לפני gate ל־**`backend`**, fallback ל־**`backend:latest`** אם תג SHA חסר ב־GHCR או ב־rollback. **Env hardening:** סנכרון `REDIS_PASSWORD` ל-`chat-ws/.env`, הזרקת `POSTGRES_HOST_DIRECT`/`POSTGRES_PORT_DIRECT` (אם חסרים), מחיקת `REDIS_SENTINEL_HOST`, דדופליקציה של `PGBOUNCER_ADMIN_PASSWORD`, ו-**`--remove-orphans`** ב-compose up הראשון (ניקוי קונטיינרים יתומים מגרסאות קודמות). פירוט: **[`docs/DEPLOYMENT.md`](DEPLOYMENT.md)**, **[`README.md`](../README.md)**.

- **Passenger ride search — temporal filters + clean cursor:** **`RideSearchRequest`** / **`GET …/search-rides`**: **`departure_date`** (Jerusalem calendar day ב־UTC window), **`departure_time`** (±2h), או טווח סגור עם **`departure_time_to`**; **הדדיות** → **422**. pagination עברה ל-**opaque cursor** (`after`/`next_cursor`) עם keyset ישיר על `(departure_time, ride_id)` (ללא lookup ביניים לפי `ride_id` raw), ונוסף **`destination_radius`** אופציונלי לרדיוס יעד נפרד. CRUD **`find_rides_by_coordinates`**; פרונט **`buildManualRideSearchParams`** + **`buildParamsFromAiResult`** לפי **`searchMode`**; Orval מאותו openapi snapshot; טסטים: **`tests/domain/test_ride_search_request.py`**, **`rideSearchParams.test.ts`**.
...
- **Rate limiting split by threat model (Token Bucket + Sliding Window):** המימוש שודרג מ-fixed window (`INCR+EXPIRE`) לשני Lua scripts אטומיים: `rate_limit_auth` עם Sliding Window (anti-bruteforce ללא burst) ו-`rate_limit_chat` עם Token Bucket (burst-tolerant לפרודוקט UX), כולל `X-RateLimit-*` headers ו-fail-open ב-Redis outage. **Coverage:** כל auth endpoints מוגנים — `register`, `login`, `refresh`, `resend-verification`, `password-reset/request`, `google-signin`, **`verify-email`** (OTP brute-force), **`password-reset/confirm`** (OTP brute-force). **Rides:** `rate_limit_rides` — Sliding Window פר-משתמש, מקסימום **10 יצירות נסיעה לשעה** (`RATE_LIMIT_RIDES_WINDOW_SECONDS` / `RATE_LIMIT_RIDES_MAX_PER_WINDOW`); מוחל על `POST /rides/` (יצירה) ו-`POST /rides/preview-routes` (preview — מפעיל Google Maps). ראו [FEATURE_DECISIONS.md — rides-rate-limit](FEATURE_DECISIONS.md#rides-rate-limit).
...
- **Persistent audit log (admin + billing + auth):** נוספה טבלת `audit_log` עם אינדקסים לפי actor/resource וזמן, repository ייעודי, וכתיבה מהפעולות האדמיניות הרגישות (`toggle_user_active`, `toggle_user_admin`, `cancel_ride`, `outbox_requeue`) בנוסף ל-`[admin_audit]` בלוגים. ב-billing webhook (`checkout.session.completed`) audit attempt נכתב **לפני** בדיקת idempotency לפי `stripe_event_id`, כך שגם retries/duplicates מתועדים פורנזית. **Auth:** 5 endpoints (`login`, `logout`, `change_password`, `google_signin`, `password_reset/confirm`) כותבים ל-audit log עם IP (מ-`X-Forwarded-For`), email, ו-provider; כשלונות login/google נתפסים ב-try/except ונרשמים כ-`login_failed` עם `reason=error_code` לפני re-raise. `client_ip` חולצה מ-`rate_limit.py` ל-shared helper (`app/api/helpers.py`) — מקור אמת אחד ל-IP extraction.
...
- **Stage 3b Part 2 — React Query migration (MyBookings Driver + Passenger):** `useMyBookingsPassenger` ו-`useMyBookingsDriver` עברו מ-fetch ידני ל-React Query: **פעיל** ב-`useQuery` (`qk.bookings.driverActive` / `qk.bookings.passengerActive`), **היסטוריה** ב-`useInfiniteQuery` (`…/driverHistory` / `…/passengerHistory`) מול **`GET …/driver-summary/history`** ו-**`…/passenger-summary/history`** (`after` / `next_cursor`). ה-endpoints הישנים **`/driver-summary`** ו-**`/passenger-summary`** נשארים לתאימות אך **deprecated** ב-FastAPI. Mutations (approve/reject/cancel ride) מבצעות invalidate לשני המפתחות (פעיל + היסטוריה). שמירה על `driverStatus` state machine, `useUserEvent`/`useLocationBroadcast`, וחוזה `useMyBookings` / **`MyBookingsViewModel`** (`activeItems` / `historyItems`, כפתור «טען עוד»). בבקאנד: תקרת **200** לפעיל; היסטוריית נהג עם `with_loader_criteria` מורחב ל-manifest היסטורי — ראו **`docs/architecture/DATABASE.md`**.
...
- **OpenAPI contract codegen (Orval) — auto-export pipeline:** מקור אמת יחיד הוא **`app.openapi()`** ב-FastAPI ([`app/main.py`](../backend/app/main.py)). Workflow ייעודי [`.github/workflows/openapi-contract.yml`](../.github/workflows/openapi-contract.yml) מייצא טרי בכל ריצה דרך [`backend/scripts/export_openapi.py`](../backend/scripts/export_openapi.py) (`indent=2, sort_keys=True, ensure_ascii=False`), ואז `npm run gen:api` ובדיקת `git diff --exit-code -- frontend/src/api/generated/`. ה-snapshot עצמו (`frontend/openapi-snapshot.json`) הוא **build artifact ב-`.gitignore`** — לא מקומיט יותר, מה שמסיר כפילות מקור-אמת. ה-workflow רץ ללא Postgres/Redis/RabbitMQ (`app.openapi()` lazy ולא נוגע ב-IO; `setup_admin`/Firebase init lazy-friendly). DX מקומי: `make openapi` בשורש, או `npm run openapi:sync` מ-frontend. ראו [FEATURE_DECISIONS — Stage 2](FEATURE_DECISIONS.md#openapi-snapshot-code-generation-orval).
...
- **Stage 3d — Chat RQ migration (safe subset):** בוצעה מיגרציה מדורגת לשכבות polling/fetch בלבד בלי big-bang: `useChatUnreadMessages` עבר מ-`setInterval` ל-`useQuery` (`qk.chat.unread`), `useChatNotificationsFeed` עבר מ-`setInterval` ל-`useQuery` (`qk.notifications.all`) עם invalidate refresh API, ו-`Messages.tsx` משתמש ב-`useInfiniteQuery` + **`GET /chat/conversations?limit&after`** (`PaginatedConversationsResponse`, cursor Base64) + `IntersectionObserver` לטעינת עמודים (מפתח: `['chat','conversations','list', limit]`). שכבות WS הקריטיות (`useConversationMessages`, `useChatPopup`, `useChatWebSocket`, `processChatWebSocketMessage`) נשארו React state מקומי (לא RQ) — עם **`ChatListRow[]`** + optimistic send + reconciliation (ראו bullet “Chat outbound” למעלה). **תשומת לב:** מיגרציות **Stage 3b** מרכזיות (**`MyRides.tsx`**, **`MyBookings`**, חיפוש/בקשות נוסע ב־RQ) מתועדות ומסומנות ב־**[`docs/FRONTEND_UPGRADE_ROADMAP.md`](FRONTEND_UPGRADE_ROADMAP.md)**; שם גם הפערים (ניהול קבוצה, העלאות, `CreateRide`, צ’אט מלא וכו׳).
...
- **Stage 5 cleanup — MyRequests + Auth bootstrap effect:** `useMyRequests` הועבר ל-React Query (`qk.passengers.requests`, cancel mutation עם `setQueryData`, ו-`REQUEST_EXPIRED` cache patch), וחוזה ההחזרה נשמר תואם ל-`MyRequests.tsx`. בנוסף תוקן bug ב-`AuthContext` initial `useEffect` (dead mounted check) — כעת משתמש ב-`AbortController` עם abort on unmount (שודרג מ-`cancelled` boolean ב-M2) — שיפור יציבות boot ללא שינוי API חיצוני.
- **Auth render gate — race condition fix:** תוקן race condition ב-page refresh: `AuthProvider` היה מרנדר children מיד (כולל `GroupProvider`, `ChatProvider` וכל ה-hooks שלהם) בזמן ש-bootstrap עדיין async — `useChatUnreadMessages` עלול היה לשלוח `GET /chat/unread-count` לפני שה-auth bootstrap סיים, ולקבל 401. **תיקון:** (1) `isLoading` מאותחל סינכרונית מ-`localStorage` (lazy `useState` initializer) — `true` רק כשיש token; (2) `AuthProvider` מציג `<PageLoading />` כל עוד `isLoading` ולא מרנדר children — מבטיח שאף provider/hook צאצא לא עולה לפני שה-auth state מוכן. עמודים ציבוריים (login, register) ללא token מרונדרים מיד ללא flash.
...
- **i18n dead-key cleanup:** סריקה אוטומטית של 430 מפתחות תרגום מול כל קבצי המקור; הוסרו 39 מפתחות לא בשימוש (9.1%) מ-`he`/`en` ב-`src/i18n/locales/` וב-`public/locales/` — מפתחות שהוחלפו (`resendCode` → `resendNewCode`, `noBookingsPassenger` → `noPassengerBookings` וכו'), מפתחות שגיאה שלא נקראים (`err_load_requests`, `err_load_passenger_bookings`, `err_load_driver_bookings`), ותוויות UI ישנות (`rideDetails`, `routePreview`, `rating`, `saveChanges` ועוד).
...
- **ConfirmModal aria fix (M12):** הוסר `aria-hidden="true"` מ-backdrop wrapper ב-[`ConfirmModal.tsx`](../frontend/src/components/ConfirmModal/ConfirmModal.tsx) — ה-attribute הסתיר את כל ה-dialog מ-assistive technology. `aria-modal="true"` על ה-panel הפנימי מטפל נכון בסמנטיקה של הסתרת התוכן שמאחורי המודל.
...
- **H20 — Google Sign-In XSS-safe React fallback (senior refactor):** `useGoogleSignIn.ts` שוכתב: הוסר `buildFallbackButton` (55 שורות `createElement`/`appendChild` — אנטי-פטרן ב-React); כפתור fallback עכשיו מרונדר כ-React component (`<GoogleFallbackButton />`) ב-`GoogleSignIn.tsx` לפי state flag `fallback` מה-hook. **Triple try/catch** שטוח ל-single try/catch נקי. **Timeout detection** מאוחד — `isTimeoutOrAbortError` מ-`utils/apiError.ts` (אותו util שכבר בשימוש ב-`Login.tsx`) במקום inline check כפול. **`while(firstChild) removeChild`** נשאר — Google GIS owns that DOM node, אין חלופה React-ית.
- **M7 — `/my-bookings` paginated frontend client alignment:** `fetchMyBookings` ב-`frontend/src/api/bookings.ts` עודכן להתאמה ל-`PaginatedBookingsResponse` מהבקאנד (`{ items, total, page, limit, has_more }`); מקבל `{ page?, limit?, status? }` במקום positional args; ברירת מחדל `page=1, limit=20`.
...
- **Stage 2 hardening batch (security + resilience + logging hygiene):**
  - **Open redirect protection (C6):** `Login.tsx` ו-`useGoogleSignIn.ts` מוודאים שפרמטר `?from=` הוא נתיב מקומי (`startsWith('/')` && `!startsWith('//')`) לפני navigate — מנקודת קצה רע, המשתמש מנותב ל-`/choose-destination` כברירת מחדל.
  - **`model_validator` ל-SECRET_KEY (H1, M1):** `config.py` משתמש ב-Pydantic v2 `model_validator(mode="after")` (לא `field_validator`). **SECRET_KEY חייב להיות non-empty בכל סביבה** (dev/staging/production) — ריק = crash מיידי ב-startup; בנוסף, ≥32 תווים נאכף ב-`ENVIRONMENT=production`. חתימת JWT עם מחרוזת ריקה כבר לא אפשרית בשום מצב.
  - **Outbox MAX_RETRIES cap (H3):** `MAX_RETRIES = 5` ב-`outbox_service.py`; אירוע שחרג מ-5 ניסיונות מסומן `FAILED` ב-DB ולא חוזר ל-PENDING — מונע retry storms אינסופיים. שחזור ידני דרך ממשק אדמין (outbox requeue).
  - **Logging hygiene (H5):** הוסרו קריאות `print()` מ-`OutboxRepository.save_event`, מ-`register` endpoint ב-`router.py`, ומ-`domain/chat/calendar/exporter.py` (הוחלף ב-`logger.error`) — `print` לא משתלב עם מערכת הלוגים המובנית (JSON/structured).
- **PgBouncer (production-ready, Compose internal):** נוסף service ייעודי `pgbouncer` (transaction pooling) בין `backend`/workers לבין `db`, ללא חשיפת פורט חיצוני. שירותי runtime הסטנדארטיים **`backend`**, **`notification-worker`**, **`task-worker`**, **`ai-worker`** מחוברים דרך `POSTGRES_HOST=pgbouncer`. **`migrate`** נשאר direct ל־**`db`** כדי למנוע friction ב-DDL/migrations. נוספה התאמת דרייבר ב-`session.py`: `connect_args={"statement_cache_size": 0}` עבור asyncpg + PgBouncer. בעקבות בעיות entrypoint ב-images ציבוריים, ה־service עבר ל־custom image (`infrastructure/pgbouncer/Dockerfile`) עם control מלא על `pgbouncer.ini`. **Outbox LISTEN/NOTIFY** ב-`run_outbox_worker` משתמש ב-**`DATABASE_URL_DIRECT`** (`POSTGRES_HOST_DIRECT` / `POSTGRES_PORT_DIRECT`) כדי לעקוף את ה-pooler — אחרת NOTIFY לא מגיע וה-worker נשאר על polling גס.
- **Single Redis reliability (Stage 2, Compose-ready):** Redis רץ כ-service יחיד `redis` עם persistence (`appendonly yes`, `appendfsync everysec`, RDB snapshots) וחלוקת DB לוגית: **DB 0** ל-cache/rate-limit/idempotency/denylist ו-**DB 1** לצ'אט/pubsub/presence. Sentinel הוסר כי ב-single-host EC2 הוא לא נתן HA אמיתי ברמת host והוסיף מורכבות תפעולית; `chat-ws` משתמש ב-`go-redis` `NewClient` דרך `REDIS_ADDR`.
- **chat-ws concurrency safety (Go):** (1) כל Redis subscribers (`RunSubscriber`, `RunUserOfflineSubscriber`, `RunUserOnlineSubscriber`) עטופים ב-reconnect loop עם exponential backoff (1s → 30s cap) — ניתוק Redis חולף לא הורג את ה-goroutine לצמיתות; (2) כל `Conn` מחזיק `done chan struct{}` שנסגר ב-disconnect; `RunWritePump` מאזין ל-`done` (לא סגירת `Send`), וכל שולח (`SendToUser`, `broadcastOnline`, `broadcastOffline`) בודק `<-c.done` ב-select — מונע panic של `send on closed channel` כשברודקאסט רץ במקביל ל-connection teardown. (3) `GET /rides/{ride_id}` ב-backend קיבל אימות `get_current_user` שחסר (חשיפת מידע ללא auth). (4) `datetime.now()` (naive, timezone-dependent) הוחלף ב-`datetime.now(UTC)` בכל שאילתות spatial/temporal וב-`domain/chat/calendar/time_parser.py` כדי למנוע באגים בהשוואת TIMESTAMPTZ ובחישובי תאריך.
- **Backend CD rollout on single EC2 (low-downtime):** **[`deploy-ec2.yml`](../.github/workflows/deploy-ec2.yml)** מריץ לאחר CI מוצלח על `main` (אחד מארבעת ה-workflows) — SSH (`appleboy/ssh-action`), משיכת תמונות (`sha` עם fallback ל-`latest`), rollout ל-backend (`docker compose up -d --no-deps backend`) אחרי smokes פנימיים ל-`frontend` + `email-renderer`, ואז smoke-gate: `sleep` קצר, **`/readyz`** פנימי, `FIREBASE_CREDENTIALS_JSON`, ו-Redis ב-`chat-ws`. rollback אוטומטי לתג קודם; אם `docker pull` לתג הישן נכשל — **`backend:latest`**.
- **Rate limiting hardening (atomic + correct-by-threat):** הוחלף fixed-window לא אטומי (`INCR + EXPIRE`) בשני Lua scripts אטומיים: `sliding_window` ל-auth (ללא burst, anti-bruteforce) ו-`token_bucket` לצ'אט (burst-tolerant API throttling). החריגה `RateLimitExceeded` הועשרה ונוספו headers סטנדרטיים `X-RateLimit-Limit/Remaining/Reset` + `Retry-After`, יחד עם metrics ייעודיים (`rate_limit_rejected_total`, `rate_limit_redis_errors_total`, `rate_limit_evaluation_seconds`). **Rides:** נוסף `rate_limit_rides` — Sliding Window פר-משתמש (10 יצירות לשעה) על `POST /rides/` ו-`POST /rides/preview-routes`.
...
- **Chat message push — offline fallback (presence-gated + debounce):** הודעות צ'אט עברו רק דרך Redis pub/sub → chat-ws WS — אם הנמען offline, ההודעה נבלעה. **פתרון:** `chat.message_sent` outbox event עובר ל-**dual-target**: `REDIS` (real-time WS) + `RABBITMQ` (push fallback). **`handle_chat_message_push`** ב-[`notification_tasks.py`](../backend/app/workers/tasks/notification_tasks.py) בודק: (1) **Presence** — `EXISTS presence:{recipient}` ב-Redis DB 1 (מנוהל ע"י chat-ws Go; 60s TTL); online → דילוג. (2) **Debounce** — `SET NX` עם TTL 30s פר שיחה; מקסימום push אחד ל-30 שניות באותה שיחה. (3) **Dispatch** — `NotificationCommand` עם template `chat_message`, channel `push` בלבד, דרך `NotificationManager`. **SW collapsing:** FCM data payload כולל `conversation_id`; ה-service worker משתמש ב-`tag: 'chat-' + conversation_id` + `renotify: true` — הדפדפן מחליף (לא מערם) התראות לאותה שיחה. אפס שינויים ל-Go chat-ws / `NotificationEvent` enum. פירוט: [`FCM_SYSTEM_SUMMARY.md` §9](FCM_SYSTEM_SUMMARY.md), [`FEATURE_DECISIONS.md#chat-message-push`](FEATURE_DECISIONS.md#chat-message-push).

- **FCM token lifecycle hardening (frontend + backend):** (1) **PushProvider return-not-raise:** `UnregisteredError`/`SenderIdMismatchError` are expected lifecycle events — provider now clears the token in DB and **returns cleanly** (was: re-raise causing `NotificationManager._safe_send` to log a false `"❌ push failed"`). Log level lowered from `warning` to `info`. (2) **localStorage token cache:** `initFCM()` compares `getToken()` result against `localStorage('fcm_token')`; if unchanged, skips `PATCH /users/fcm-token` — saves a redundant HTTP call on every page reload (FCM tokens are stable for weeks). Cache is cleared inside `cleanupFCM()` (not in AuthContext) — full encapsulation in `fcm.ts`, and prevents a multi-user bug where a stale cache from a previous user's expired session would cause the next user to skip their PATCH. Decision rationale: [`FEATURE_DECISIONS.md#push-provider-return-not-raise`](FEATURE_DECISIONS.md#push-provider-return-not-raise), [`FEATURE_DECISIONS.md#fcm-token-localstorage-cache`](FEATURE_DECISIONS.md#fcm-token-localstorage-cache).

- **Unified auth session teardown (frontend web):** [`tearDownSession({ reason })`](../frontend/src/context/AuthContext.tsx) מאחד **logout משתמש**, **כישלון bootstrap** (**`refreshUser`** / hydrate ראשוני), ו-**כישלון refresh** בפרונט. **`reason: user-action`** — `patchFcmToken(null)` + **`POST /auth/logout`** (בזמינות JWT תקף), לאחריו **`cleanupFCM`**, **`queryClient.clear()`**, **`Sentry.setUser(null)`** (PROD), **`clearTokens`**, **`isAuthenticated=false`**. **`session-expired`** / **`bootstrap-failed`** — רק ניקוי מקומי (אין PATCH/logout עם JWT מת). **[`client.ts`](../frontend/src/api/client.ts)** ב-**`refreshAccessToken`** — שולח `POST /auth/refresh` עם **`withCredentials: true`** (ה-refresh token נשלח אוטומטית כ-HttpOnly cookie); בעת catch: **`clearTokens`** + **`auth:session-expired`** עם **guard reentrancy**; מסימון **`__sentryCaptured`** על 401 אחרי כשל רענון (defense-in-depth). [`queryClient.ts`](../frontend/src/api/queryClient.ts) — **`captureExceptionOnce`** מדלג על **401** בלבד (לא **403**). פירוט: **[FEATURE_DECISIONS.md](FEATURE_DECISIONS.md#auth-session-teardown)**, **ADR Frontend §21**.
- **H19 — Refresh token in HttpOnly cookie (XSS mitigation):** ה-refresh token לא חוזר יותר ב-JSON ולא נשמר ב-localStorage. **בקאנד:** `_set_refresh_cookie(response, token)` קובע `Set-Cookie: linkup_refresh_token` עם **`HttpOnly; Secure; SameSite=lax; Path=/api/v1/auth; Max-Age=REFRESH_TOKEN_EXPIRE_DAYS*86400`** ב-login, google-signin, refresh; `_clear_refresh_cookie` ב-logout; `POST /auth/refresh` קורא מ-`request.cookies` (401 אם חסר). **פרונט:** axios instance עם **`withCredentials: true`**; `setTokens(access)` ללא refresh param; `clearTokens()` מנקה גם legacy `linkup_refresh_token` key מ-localStorage (one-time migration); `refreshAccessToken()` שולח body ריק — הדפדפן שולח את ה-cookie אוטומטית. **הגנת CSRF:** `SameSite=lax` + path scope (`/api/v1/auth`) + POST-only endpoints = אין GET משנה state. **`LoginResponse`/`RefreshResponse`** schemas הוסרו מ-`refresh_token` field. פירוט: **`docs/adr/ARCHITECTURE_DECISIONS_BACKEND.md`**, **`docs/FEATURE_DECISIONS.md`**.
...
- **Passenger match notifications (email + push) documented end-to-end:** Outbox publishes **`ride.created`**; `notification_tasks.handle_ride_created` runs `find_passengers_for_ride_notification`; per-match notification uses internal event **`ride.created_for_passengers`** (not a second Rabbit routing key) on **`["email", "push"]`** channels. See §6.4 below and **`architecture/EVENTS.md`** (Ride section).
...
- **Outbox dispatch improved**: LISTEN/NOTIFY flow is now primary (listener uses **`DATABASE_URL_DIRECT`** to connect straight to Postgres, not PgBouncer), with safe fallback behavior to avoid fixed-interval-only polling when the listener is unavailable.
- **Worker split completed**: `notification-worker`, `task-worker`, `ai-worker` each has dedicated runtime responsibilities as separate Compose services.
...
- **Postgres `statement_timeout` (layered, 017):** ערך אופרטיבי ברמת **session** מ-`DB_STATEMENT_TIMEOUT_MS` (default 30s) דרך `connect_args.server_settings` ב-[`app/db/session.py`](../backend/app/db/session.py) — **ה-env באמת משפיע ב-runtime**. Alembic **017** מוסיף **ceiling דיפנסיבי קשיח של 60s** ברמת role דרך `ALTER ROLE CURRENT_USER` (literal, ללא תלות ב-`settings` — מיגרציה דטרמיניסטית). Defense-in-depth: גם אם משהו ייצור connection ישיר שעוקף את ה-engine, הוא לא יוכל לחרוג מ-ceiling. שאילתות ארוכות אובורטות ב-Postgres לפני Gunicorn timeout; **`docs/architecture/DATABASE.md`**.
- **Ride list hard cap + indexes (018):** driver and group ride list queries cap at **200** rows (`_RIDES_HARD_LIMIT` in **`rides/crud.py`**), sorted by `departure_time` DESC; **`idx_rides_driver_departure`** and partial **`idx_rides_group_departure`** replace **`idx_rides_driver_id`** / **`idx_rides_group_id`**. Documented in **`docs/architecture/API.md`** and **`docs/architecture/DATABASE.md`**.
- **Composite / partial indexes for hot CRUD paths (022):** 6 indexes targeting frequent filter combinations — **`idx_bookings_passenger_status`** (`get_passenger_active_bookings`), **`idx_bookings_ride_status`** (`get_ride_bookings_by_status_async`), **`idx_passenger_requests_status`** + **`idx_passenger_requests_passenger_status`** (search + `get_my_requests`), partial **`idx_outbox_events_status_created`** (pending events polling, `WHERE status='PENDING'`), partial **`idx_users_last_active_at`** (presence queries, DESC, `WHERE last_active_at IS NOT NULL`). Documented in **`docs/architecture/DATABASE.md`**.
- **Redis reconnect resilience**: reconnect retry strategy uses exponential backoff (1s→30s cap) for long-lived pub/sub channels, with **automatic reset** after stable connections (backoff returns to 1s if the subscription ran longer than the max backoff window).
...
- **Chat reliability/UX**: chat message reads are now split into two clean contracts. **History pagination** uses `GET /chat/conversations/{id}/messages` with opaque keyset cursor (`after`) over `(created_at, message_id)` and deterministic SQL ordering `created_at DESC, message_id DESC`. **Reconnect backfill** uses dedicated `GET /chat/conversations/{id}/messages/gap?since_message_id=` and repeats only while `truncated=true`, advancing with `last_message_id`. In frontend, `useConversationMessages` passes opaque cursor as-is (no numeric parsing), and `fetchMissedGap` handles reconnect batches separately with `shouldAbort` safety when `conversation_id` changes. **Scroll behavior is intent-based:** a `scrollIntentRef` (`'bottom'`/`'preserve'`/`'none'`) is set before each `setMessages` call and consumed in a `useLayoutEffect` — initial load/sends force bottom, load-older preserves offset, gap-fill/inbound WS scroll only if near bottom. Message dedupe/merge stays `message_id`-based (`applyInboundRealMessage`, `appendMessageDedupById`) and read receipts still rely on DB-level `last_read_message_id` + Redis `message_read` with `recipient_id`.
...
- **`useUserEventStream` framing:** inbound WS text is parsed as a **single JSON object per frame** — each message is an independent WebSocket text frame from chat-ws (`WriteMessage` per message in `RunWritePump`). הוק מטפל ב־**`InvalidateEvent`** ואז ב־**`UserEvent`** (סדר קבוע).
- **FCM service worker:** background handling uses **`messaging.onBackgroundMessage`** (primary) reading **`payload.data`** (matches backend data-only pushes) + a raw **`self.addEventListener('push', ...)`** fallback for browsers where `onBackgroundMessage` does not fire for data-only messages (notably Chrome with closed tab). Firebase suppresses the raw event when `onBackgroundMessage` handles the message — no duplicate notifications.
- **Shared cursor infrastructure + chat inbox pagination:** cursor helpers אוחדו ל-**`backend/app/core/pagination/cursor.py`** (UTC normalization + opaque Base64 JSON), ו-`list_conversations_paginated` (`chat/crud.py`) ממשיך למיין לפי `COALESCE(conversations.last_message_at, conversations.created_at)` + `conversation_id`, `limit+1` ו-keyset אחרי cursor; אגרגציות נשארות ב-**`get_inbox_aggregates`** רק על ה-IDs בעמוד; **`list_conversations_for_user`** נשמר לשימושים אחרים.

- **Passenger spatial match cap + notifications cursor feed + cancel-ride status batch + groups member counts:** התאמות מיידיות ב-**`create_passenger_request`** / **`get_matches_by_request_id`** מוגבלות ב-**`_IMMEDIATE_MATCH_LIMIT = 20`** (`passengers/service.py`) כדי לא לרוץ על `find_rides_by_coordinates` בלי תקרה. פיד in-app **`GET /users/me/notifications`** עבר ל-**cursor pagination** (`limit`, `after` → `items`, `next_cursor`, `has_more`) עם keyset יציב **`created_at DESC, booking_id DESC`**, כולל מיזוג שני מקורות (`get_all_pending_bookings_for_driver` + `get_user_bookings_with_relations`) אחרי סינון cursor בשכבת CRUD. אחרי **`cancel_ride_and_bookings`** — סטטוס **`passenger_requests`** מחושב מ-**`_status_from_bookings_list`** (מקור יחיד שממנו גם **`determine_passenger_request_status`**) + עדכון bulk לפי סטטוס. רשימת „הקבוצות שלי” משתמשת ב-**`get_member_counts_batch`** (`groups/crud.py`) במקום N+1 על **`get_member_count`**. פירוט: [FEATURE_DECISIONS.md — API read caps & batch status](FEATURE_DECISIONS.md#api-read-caps-batch-status).

- **Passenger cancel request — O(1) DB path + `booking_status` enum parity:** ביטול בקשה (**`PassengerService.cancel_request`**) מריץ **`bulk_cancel_bookings_for_request`** (`bookings/crud.py`): אגרגציה של מושבים לפי נסיעה (סטטוסים שתפסו מושב: **`confirmed` / `en_route` / `arrived` / `trip_in_progress`**), נעילת שורות **`rides`** עם **`WITH FOR UPDATE`**, עדכון bulk של כל ה-**`bookings`** של הבקשה — במקום לולאה עם **`update_passenger_request_status_from_bookings`** לכל שורה (שמנעה N² ועדכוני **`passenger_requests`** מיותרים לפני **`CANCELLED`**). מיגרציה Alembic **`019_booking_lifecycle_enum`** מוסיפה את שלושת ערכי מחזור הנסיעה ל-PostgreSQL **`booking_status`** (לפני כן ה-ORM שידר למסנן `IN (...)` ערכים שלא היו ב-enum). טסטים: **`backend/tests/test_bookings.py`**. **`BookingService.cancel_all_bookings_for_request`** מאוחד לאותו נתיב + **`UPDATE passenger_requests`**. פירוט: [DATABASE.md](architecture/DATABASE.md) (מיגרציות + Race), [API.md](architecture/API.md) (DELETE cancel), [FEATURE_DECISIONS.md](FEATURE_DECISIONS.md#api-read-caps-batch-status).
...
- **Chat detail / booking-chat — redundant conversation re-fetch elimination:** `get_or_create_conversation`, `get_or_create_conversation_by_booking`, and `get_conversation_detail` (`chat/service.py`) each re-fetched the conversation **2 extra times** via `_get_partner_last_read_at` and `_get_partner_read_up_to_message_id` (each internally called `get_conversation_by_id` to compute `partner_id`). Replaced with a single `_get_partner_read_info(db, conversation_id, partner_id)` that fetches both `last_read_at` and `last_read_message_id` in **one query**, using the `partner_id` already available in scope. Result: each endpoint drops from 5–7 queries to 2–4. See [FEATURE_DECISIONS.md — Chat detail redundant re-fetch](FEATURE_DECISIONS.md#chat-detail-redundant-refetch).
- **Pending booking requests — eager loading fix:** `GET /bookings/ride/{ride_id}/pending` returned bookings with `passenger_name: null` because `get_ride_bookings_by_status_async` loaded bare `Booking` rows without relationships. Added `get_ride_bookings_by_status_with_relations` (`bookings/crud.py`) with `joinedload(Booking.passenger_request → PassengerRequest.user)` + `joinedload(Booking.passenger)`; `get_pending_requests` now uses it while lightweight callers (`start_ride`, `broadcast_driver_location`) keep the original method.
- **Task scheduler safety**: `task-worker` is fixed to a single replica to prevent duplicate scheduled task publishing.

- **Transaction ownership refactor (CRUD flush-only):** `CRUDUser` write methods no longer call `db.commit()` — all 7 methods (`update`, `update_location`, `update_fcm_token`, `update_refresh_token`, `update_password`, `update_last_active`, and the removed dead-code `mark_as_verified`) now use `db.flush()` only, consistent with `create`, `mark_as_premium`, and `update_stripe_customer_id` which already followed this pattern. Callers in `auth/service.py` (6), `push_provider.py` (1), `users/service.py` (2), `users/router.py` (1), and `chat/router.py` (1) now own the transaction boundary with explicit `await db.commit()`. This enables any caller to combine DB writes with outbox events in a single atomic transaction — the same pattern already proven in `register_new_user`, `confirm_avatar_upload`, `remove_avatar`, `billing/service.py`, and now `chat/service.send_message` (which was refactored from owning `db.commit()` to flush-only, letting `post_message` router commit message + outbox + last_active atomically). Session factory uses `expire_on_commit=False` ([`app/db/session.py`](../backend/app/db/session.py)), so no `db.refresh()` is needed after commit at caller sites. Dead-code method `mark_as_verified` (zero callers — `verify_user_email` uses generic `update()`) was removed. Two new dedicated methods added: **`link_google_account`** (sets `google_id` + `is_verified`) and **`update_last_login`** (stamps `last_login` to now) — completing the elimination of direct ORM attribute assignments in `authenticate_with_google`. Documented in ADR backend §29.
...
| פריסה | **Docker Compose** (מקומית + **EC2** דרך **`deploy-ec2.yml`**) |
...
| Push | **FCM** — `fcm_token` ב-DB; שליחה דרך Firebase Admin עם **`data` בלבד** (ללא בלוק `notification` של FCM); הצגה בידי האפליקציה: ברקע SW `onBackgroundMessage` (ראשי) + raw `push` listener (fallback); בחזית **Toast + צליל** (`onMessage` + `payload.data`) |
...
| **`task-worker`** | consumers ל-`avatar_upload_queue` ו-`scheduled_tasks_queue`, plus scheduled publisher loop כל ~**60 שניות**; משימות scheduled כוללות **chat idle timeout** → `handle_conversation_completion` (**Groq** via `asyncio.to_thread` — non-blocking → `chat_analysis` + Outbox). |
...
2. **notification-worker** מפרסם ל-Redis `chat:conversation:{id}` דרך `RedisChatPublisher`.
...
4. **אין** יצירת הזמנה אוטומטית; ההתראה (מייל + push) מזמינה את הנוסע לפתוח את האפליקציה / לחפש שוב.
...
| **Chat detail — redundant re-fetch elimination** | `_get_partner_read_info` (`chat/service.py`) — helper יחיד שמחליף שני helpers שכל אחד קרא `get_conversation_by_id` מחדש; מ-5–7 queries ל-2–4 per endpoint. פירוט: [FEATURE_DECISIONS.md — Chat detail redundant re-fetch](FEATURE_DECISIONS.md#chat-detail-redundant-refetch). |
| **Pending bookings — eager loading** | `get_ride_bookings_by_status_with_relations` (`bookings/crud.py`) — `joinedload` על `passenger_request.user` + `passenger` כדי ש-`GET /bookings/ride/{id}/pending` יחזיר `passenger_name` במקום `null`. |
...
| **Rate limiting (Redis)** | על **register**, **login / refresh** ונקודות auth נוספות — מונה ב-Redis, חלון זמן + מקסימום בקשות ל-IP — מגביל הרשמה/כניסה אגרסיבית; בצ'אט יש rate limit פר-משתמש על `POST /chat/conversations/{conversation_id}/messages` (30 הודעות/דקה, fail-open אם Redis לא זמין); **נסיעות** — `rate_limit_rides` פר-משתמש (10 יצירות/שעה, Sliding Window) על `POST /rides/` ו-`POST /rides/preview-routes`. |
...
| **תשתית** | **`pool_pre_ping`**, **`pool_timeout`**, **`pool_recycle`** — מאגר DB עמיד יותר; **rate limit** על register + login/refresh; **FCM** — טוקן לא תקף: דילוג בשליחה אם אין טוקן; איפוס **`fcm_token`** ב-DB כש-Firebase מחזיר רישום לא תקף (**`PushProvider`** — return cleanly, לא raise); localStorage cache בפרונט לדילוג על PATCH מיותר. |
...
| **Refresh token storage (M4)** | **SHA-256 hash** ב-DB — plaintext לא נשמר אף פעם. Hash ב-CRUD (`update_refresh_token`); השוואה ב-`verify_refresh_token` עם **`hmac.compare_digest`** (constant-time); **schema-derived allowlist** ב-generic `update`: `_allowed_update_fields = set(UserUpdate.model_fields.keys())` — רק שדות שמוגדרים ב-Pydantic schema ניתנים לכתיבה; שדות רגישים (`is_admin`, `is_premium`, `is_active`, `is_verified`, `refresh_token`) דורשים CRUD methods ייעודיים (`mark_as_verified`, `update_refresh_token`); migration **021** — null-ify קיימים. |
...
| סשן | JWT (HS256), `SECRET_KEY` חובה בפרודקשן; **`jti`** ל-access; denylist ב-Redis אחרי logout; **password reset/change מנקה refresh token** (M7 — `_invalidate_sessions`); אותו סוד ל-chat-ws לאימות WS. |
...
| **מאגר חיבורי DB** | **`DB_POOL_*`** — `pool_size`, `max_overflow`, `pool_timeout`, `pool_recycle`, **`pool_pre_ping`**; **`statement_timeout`** דו-שכבתי: **`DB_STATEMENT_TIMEOUT_MS`** ב-session דרך `connect_args.server_settings`, plus ceiling קשיח של 60s ב-role דרך מיגרציה **017**. | תומך בהרבה **sessions אסינכרוניות** במקביל בלי לייבש את ה-pool או להחזיק חיבורים מתים; שאילתות חריגות אובורטות ב-DB לפני workers — עם defense-in-depth: ה-env מפעיל ב-runtime, וה-ceiling חוסם גם connections שעוקפים. |
...
- **FCM (Backend)**: `app/domain/notifications/channels/push/client.py` — `messaging.Message` עם **`data` בלבד** (ללא `notification`), כולל `title` ו־`body` כמחרוזות + שדות metadata נוספים (`ride_id`, `booking_id`, `event_key`, **`conversation_id`**); שליחה ב-**`run_in_executor`** עם **`asyncio.get_running_loop()`**; **Tenacity** — retry רק על שגיאות transient של Firebase Admin (**`UnavailableError`**, **`InternalError`**, **`DeadlineExceededError`**, **`UnknownError`**), בלי retry על **`UnregisteredError`** / **`SenderIdMismatchError`**. `push_provider` שולח רק אם יש `fcm_token`; אם FCM מדווח רישום לא תקף — **ניקוי `users.fcm_token` ב-DB** דרך `crud_user.update_fcm_token` כש־**`db`** מועבר מה־handler, ואז **`return`** (לא `raise` — טוקן שפג הוא lifecycle event צפוי; לוג `info`). **Chat offline push**: `handle_chat_message_push` ב-`notification_tasks.py` — presence-gated + 30s debounce → template `chat_message` → push channel only. מייל/WebSocket מתעלמים מ־`db`.
- **FCM (Frontend)**: `frontend/src/services/fcm.ts` — הרשאות, רישום SW, `getToken` + **localStorage cache** (משווה מול `localStorage('fcm_token')`; דילוג על PATCH אם הטוקן לא השתנה) + `PATCH /users/fcm-token`; **`cleanupFCM()`** מבטל `onMessage` **ומוחק את ה-cache** (מונע באג multi-user). **AuthContext** — `initFCM()` **ללא תנאי** אחרי login / Google / hydrate (`initFCM` עצמה מטפלת ב-permission state — מבקשת הרשאה אם `"default"`, יוצאת אם `"denied"` או unsupported); ב־logout: `patchFcmToken(null)` (בזמן JWT תקף) → `cleanupFCM()` → `logoutSession` → `clearTokens`. Toast גלובלי ב־**`App.tsx`** (`NotificationToast`). תפריט פרופיל: הפעלת התראות דרך **`useLayoutShell`**. בחזית `onMessage` → `title`/`body` מ־**`payload.data`** → Toast + צליל; ברקע `firebase-messaging-sw.js` (**generated file**, gitignored; מקור אמת: `frontend/docker/firebase-messaging-sw.template.js`; ב-dev/build: Vite plugin `firebaseSwPlugin` ב-`vite.config.ts`; ב-Docker: `envsubst`) — **`messaging.onBackgroundMessage`** (ראשי) עם **`payload.data`** + raw **`push`** listener כ-fallback לדפדפנים שלא מפעילים `onBackgroundMessage` עבור data-only (Chrome עם טאב סגור); Firebase מדכא את ה-raw event כש-`onBackgroundMessage` מטפל — ללא כפילויות. **Chat notification collapsing**: כשה-data payload כולל `conversation_id`, ה-SW מוסיף `tag: 'chat-' + conversation_id` + `renotify: true` — הדפדפן מחליף התראות לאותה שיחה במקום לערום. פירוט: **`docs/FCM_SYSTEM_SUMMARY.md`**.
...
- **Docker Compose**: healthchecks (כולל **backend** על **`GET /readyz`** בתוך הקונטיינר — readiness מול DB/Redis/RabbitMQ לפני ש-**`service_healthy`** משחרר תלות; **שלושת ה-workers** (`notification-worker`, `task-worker`, `ai-worker`) עם PID-based healthcheck (`os.kill(1, 0)`) לזיהוי crash של התהליך הראשי), סיסמת Redis, volumes ל-RabbitMQ ו-Postgres; שירות **`migrate`** (`alembic upgrade head`, `restart: no`) לפני **backend** וכל ה-workers; שירותי פיתוח (`db`, `redis`, `rabbitmq`, `migrate`, `backend`, `notification-worker`, `task-worker`, `ai-worker`, `chat-ws`) ב־`docker compose up -d`; **frontend** סטטי + **nginx** באותו `docker-compose.yml` עם `profiles: ["prod"]` — סטאק מלא על פורט 80 עם `docker compose --profile prod up -d --build`, **nginx** אחרי **backend** ב־`service_healthy`. בפרודקשן Firebase עובד ב-Model B: `FIREBASE_CREDENTIALS_JSON` דרך `backend/.env` (ללא credentials file mount), ו-`FIREBASE_SERVICE_ACCOUNT_PATH` נשאר fallback לפיתוח מקומי בלבד. **פריסה בלי Compose** (למשל image בלבד / orchestration חיצוני): להריץ מיגרציה כ־Job או שלב init נפרד — לא מוטמע ב־`CMD` של image ה-production.
- **Container resource limits (tiered profiles):** כל השירותים מוגבלים ב-**`mem_limit`**, **`mem_reservation`**, **`memswap_limit`** (= `mem_limit` → מניעת swap), **`cpus`**, **`pids_limit`** דרך YAML extension fields (`x-resources-micro/light/medium/redis`) ב-`docker-compose.yml`. תקציב מותאם ל-**t3.medium** (4096MB; usable ~3500MB). Steady-state: **3264MB** (headroom ~236MB ל-page cache). פרופיל monitoring מוסיף 768MB — מתאים ל-t3.large או שימוש זמני בלבד. פרופילים הם single source of truth: שינוי tier אחד מעדכן את כל השירותים בו.
...
- **chat-ws env:** ב־Compose מוגדרים למשל **`BACKEND_URL`** (REST ל־last-seen) דרך **`docker-compose.yml`** / env files.
...
| נושא | מכוסה |
|------|--------|
| התראת נוסע על נסיעה חדשה (Outbox `ride.created` → handler → מייל + push) | **סעיף 6.4** + `architecture/EVENTS.md` |
...
| מה | פירוט |
|----|--------|
| **GitHub Actions — workflows עם path filters** | **`backend-ci`**: Ruff (+ format check), Postgres שירות, **`DATABASE_URL=test_db`**, **`uv sync`**, **`uv run alembic upgrade head`**, **`scripts/ops/check-migration-head.sh`**, **`tests/infrastructure/test_rabbitmq_reliability.py`**, **`uv run pytest tests/`** + build/push ל-GHCR על `main`. **`frontend-ci`**, **`chat-ws-ci`**, **`email-renderer-ci`** — טריגר לפי `paths`. פריסת **production** ל-**EC2** ב־**[`deploy-ec2.yml`](../.github/workflows/deploy-ec2.yml)** (`workflow_run` אחרי CI מוצלח) — **`docs/FUTURE_WORK.md`**. |
...
| **בדיקות worker tasks (async, mocked infra)** | `backend/tests/domain/` — **`test_avatar_tasks.py`** (upload/remove routing, happy path, failure status), **`test_chat_summary_task.py`** (Redis listener subscribe/pmessage/cancellation + completion message validation/rollback), **`test_notification_tasks.py`** (ride created fan-out + scheduled reminder, booking approved pickup/departure fallback, ride cancelled per-status filter, error resilience, `execute_reminders_job`), **`test_scheduled_tasks.py`** (publisher interval/exchange routing, task-not-due skip, exception resilience). All `pytest.mark.asyncio` with `AsyncMock` for DB/Redis/RabbitMQ — no live infra; run in CI as part of `uv run pytest tests/`. |
...
| **Refresh token hashed at rest (M4)** | `users.refresh_token` שומר **SHA-256 hash** בלבד — דליפת DB לא חושפת טוקנים תקפים; `hmac.compare_digest` ב-CRUD; migration **021**. |
| **Idempotent POST (נוסע)** | **`Idempotency-Key`** + **`SET NX`** + מטמון **201** ל־`request-ride-from-search` — דפוס Stripe; פירוט **§7ה**. |
| **Security headers + CSP (enforcing)** | Edge nginx: **`listen 443 ssl`** + **`http2 on`** (HTTP/2 multiplexing), SSL tuning extracted to **`nginx/snippets/ssl-params.conf`** (ECDHE-only AEAD ciphers, OCSP stapling, `ssl_session_cache`, `ssl_session_tickets off` for forward secrecy), HSTS (`includeSubDomains; preload`), `X-Content-Type-Options`, `X-Frame-Options: DENY`, `Referrer-Policy`, `Permissions-Policy`, `COOP/COEP` ל־OAuth popup (**`FEATURE_DECISIONS`** `#oauth-popup-coop`). Zero-downtime cert renewal via ACME challenge location on port 80 (`/var/www/certbot`). **CSP:** כותרת **`Content-Security-Policy`** מאוכפת; **`script-src`** ללא **`'unsafe-inline'`** (bootstrap ב־**`frontend/public/bootstrap.js`** לפני **`/config.js`**); **`style-src`** עם **`'unsafe-inline'`**; **`report-uri`** ל־Sentry CSP; **`frame-src`** ל־Stripe + Google Sign-In. פירוט: **`docs/SECURITY_HEADERS.md`**. |
| **Google OAuth phone collision fix (M8)** | users.phone_number → **nullable**; Google OAuth לא יוצר placeholder phone מזויף. **Partial unique index** מאפשר NULLs מרובים; רישום רגיל מחייב טלפון. Migration **025**, שינוי model + schema + uthenticate_with_google. |
| **Notification retry error classification (M3)** | NotificationManager._safe_send לא בולע שגיאות — providers מסווגים ל-**TransientNotificationError** (broker retry) ו-**PermanentNotificationError** (log + swallow). Push: FCM token invalid = permanent, network = transient. Email: circuit-open = permanent, API = transient. **Redis dedup** (TTL 24h) מונע כפילויות. |
| **Backend CSP (M4)** | SecurityHeadersMiddleware מוסיף Content-Security-Policy על כל תגובת API: default-src none; frame-ancestors none; base-uri none; form-action none (הכי מחמיר); /docs//redoc מקבלים CSP מרוכך. שלוש שכבות CSP (edge nginx / backend middleware / frontend nginx) — **docs/SECURITY_HEADERS.md**. |
...
| מה | פירוט |
|----|--------|
| **Alembic** | מיגרציות מסודרות; migration ייעודי **indexes** (rides, bookings, group_members, וכו’) + participants לצ’אט; **022** — composite/partial indexes for hot CRUD queries. |
...
| דגש | פירוט קצר |
|-----|-----------|
| **Unread צ’אט (בזמן אמת)** | `send_message` עושה flush בלבד (ללא commit); ה-router (`post_message`) מבצע commit אטומי יחיד (הודעה + outbox + last_active), ואז פותח `SessionLocal` טרי לשאיבת `get_unread_conversations_count` (ספירה מדויקת גם מאחורי PgBouncer). פרסום invalidate ל־**`user:{recipient_id}:events`** עם **`resource: unread_messages`** + **`count`**; **`useUserEventStream`** + **`setUnreadDirect`** ב־`ChatContext` מעדכנים מטמון RQ בלי חובה ל־HTTP עד רפרש הבא (polling נשאר גיבוי). |
...
| **שגיאות** | `getApiErrorMessage` / `getApiStatus` / `isTimeoutOrAbortError` ב־`utils/apiError.ts` + **Vitest** (`apiError.test.ts`). ב־hooks: fallback לשגיאות שרת עם **`apiErr('err_*')`** (מפתחות ב־`common.json`); שגיאות ולידציה צד-לקוח (geolocation, מוצא/יעד, מסלול) דרך **`t('err_*')`** מ-namespace פיצ'רי (`rides.json`). **`useCreateRide`**, **`useSearchRides`**, ו-**`Messages.tsx`** — אפס מחרוזות עברית קשיחות. **`isTimeoutOrAbortError`** בשימוש אחיד גם ב-`Login.tsx` וגם ב-`useGoogleSignIn.ts` (H20 refactor). |
| **i18n / טיפוגרפיה** | **`LangContext`** — `lang`, `dir`, **`--font-primary`**; קבצי תרגום תחת `src/i18n/locales/` (9 namespaces כולל **`admin`**, ~475 מפתחות פעילים); ב־**`*.module.css`** — `var(--font-primary)` / `var(--font-numeric)` (חריג: `LangToggle`). UI copy ב-`Messages.tsx` דרך **`common:msg_*`** / **`common:unread_count`** / **`common:user_fallback`**. מסכי אדמין (`/admin/*`) — namespace **`admin`** (bundled, ~100 מפתחות) עם `useTranslation('admin')` בכל דף. |
...
| **הזמנות שלי (VM)** | **`useMyBookings`** — קומפוזיציה מ־`useMyBookingsPassenger` + `useMyBookingsDriver`; החזרה **מקוננת** (`passenger`, `driver`, `chat`) + **`MyBookingsViewModel`**. טעינה: **`/bookings/driver-summary/active`** + **`/bookings/driver-summary/history`** (ו-**passenger** מקביל) — `useQuery` + `useInfiniteQuery`. כרטיס נוסע: **`PassengerBookingCard`**. |
