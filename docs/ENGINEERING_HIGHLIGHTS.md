...
| **פיד התראות (in-app)** | **`useChatNotificationsFeed`** — REST + polling **~5 דקות**; רענון חי מ־**`invalidate`** / **`UserEvent`** ב־**`useUserEventStream`** (מאזין יחיד ב־**`ChatContext`**, לא ב־Layout) + התאמת **`linkup:user-event`** בענף התראות. |
