...
- **Production bug fixes (H2, H6, H7-backoff, H8-types):** (1) **H2** — chat scroll-to-bottom on history load replaced with intent-based scrolling (`scrollIntentRef` with `'bottom'`/`'preserve'`/`'none'`); load-older preserves scroll position, gap-fill doesn't scroll, inbound WS scrolls only if near bottom. (2) **H6** — `http.Client` per-iteration resource leak in chat-ws fixed; shared `*http.Client` with tuned `Transport` on `Hub` struct and `HandlePresence` closure. (3) **H7-backoff** — Redis subscriber backoff in chat-ws now resets to 1s after stable connections (previously accumulated to 30s permanently). (4) **H8-types** — `user_id: int` annotations corrected to `UUID` across `users/service.py`, `exceptions/user.py`, `calendar_export.py`, `s3/service.py`; dead commented-out route removed. See [`docs/ENGINEERING_HIGHLIGHTS.md`](docs/ENGINEERING_HIGHLIGHTS.md).
- **Automated PostgreSQL backup pipeline (Phase 1):** pg_dump → AES-256-CBC encryption → S3 upload, triggered daily (03:00 UTC cron) and before every CI deploy. Docker Compose `db-backup` service under `backup` profile connects directly to `db:5432` (bypasses PgBouncer). S3 lifecycle rules: daily backups expire at 30d (Glacier at 14d), pre-deploy at 90d (Glacier at 30d). Restore script supports latest-or-specific backup retrieval with post-restore row-count sanity check. Monitoring via `scripts/ops/check-backup-health.sh` (freshness + size thresholds). Full runbook: **[`docs/BACKUP_AND_RECOVERY.md`](docs/BACKUP_AND_RECOVERY.md)**.
- **Server-side notification read-state (migration 023):** מצב קריאת התראות הועבר מ-`localStorage` (`linkup_notif_read`) שצבר נתונים ללא הגבלה לטבלת **`notification_reads`** ב-DB. PK מורכב `(user_id, booking_id, created_at)`, CASCADE על FKs; `GET /users/me/notifications` מחזיר `is_read` + `unread_count`; PATCH endpoints לסימון קריאה; מיגרציה חד-פעמית בפרונט שולחת מפתחות ישנים לשרת ומוחקת localStorage. **[`docs/architecture/DATABASE.md`](docs/architecture/DATABASE.md)**, **[`docs/architecture/NOTIFICATIONS.md`](docs/architecture/NOTIFICATIONS.md)**.
- **Realtime badges (chat + in-app):** כל הפריימים ל־נמען עוברים ב־Redis על **`user:{id}:events`** — **`invalidate`** עם **`unread_messages`** (אחרי שליחת הודעה + `count`) או **`notifications`** (אחרי outbox **`WebSocketProvider`**; חתימת **`send(..., db=None)`** תואמת **`BaseNotificationProvider`**), בנוסף ל־**`UserEvent`** מ־**`publish_user_event`**. **chat-ws** לא מנוי יותר ל־**`chat:notification:*`**. בפרונט: **`useUserEventStream`** ב־**`ChatContext`** — **`handleInvalidate`**: התראות → **`refreshUnreadNotifications`** + **`NOTIFICATIONS_REFRESH_EVENT`** + **`linkup:user-event`** (אם יש `event` ו־`user_id`); הודעות → **`setUnreadDirect`** / invalidate; **`UnreadCountEventSchema`** נשאר תחת פריימי צ'אט (`ChatPresenceEventSchema`) כדי שלא לשנות **`processChatWebSocketMessage.ts`**; **`useLayoutShell`** בלי WS — **`docs/architecture/REALTIME.md`**, **`chat-ws/ARCHITECTURE.md`**.
- **חיפוש נסיעות נוסע (`GET …/search-rides`):** פרמטרים **`departure_date`** (יום מלא **Asia/Jerusalem**), **`departure_time`** (חלון **±2 שעות**), או **`departure_time`** + **`departure_time_to`** (טווח כולל); **הדדיות** → **422**. הדפדוף עבר ל-**opaque cursor** (`after`/`next_cursor`) במקום `ride_id` raw, וה-query תומך גם **`destination_radius`** נפרד מ-`search_radius`. **פרונט:** **`buildManualRideSearchParams`** לפי **`searchMode`** (**`date_only` / `datetime` / `time_range`**) + **`buildParamsFromAiResult`** ללא `departure_time` מזויף ב־תאריך־בלבד; Orval מ־**`openapi-snapshot.json`**; בדיקות: **`backend/tests/domain/test_ride_search_request.py`**, **`frontend/.../rideSearchParams.test.ts`** — פירוט **`docs/architecture/API.md`**.
- **Notifications / FCM:** optional **`AsyncSession`** on **`BaseNotificationProvider.send`** — **`NotificationCommand.db`** מה־worker handler; **`PushProvider`** מנקה **`users.fcm_token`** על **`UnregisteredError`** / **`SenderIdMismatchError`**. **`FCMClient`**: retry על transient Firebase בלבד, **`get_running_loop()`** + executor. **Frontend container:** `env_file: ./frontend/.env` + **`40-render-config.sh`** fail-fast על מפתחות Firebase חובה + defaults ל־`APP_ENV` / timeout / Map ID; **Service Worker** `importScripts` גרסה מיושרת ל־**`firebase` npm** (ראו **`docs/FCM_SYSTEM_SUMMARY.md`**, **`docs/architecture/NOTIFICATIONS.md`**).
...
- **chat-ws operational hardening (H7–H11):** **`/healthz`** endpoint verifies Redis connectivity **and** subscriber goroutine liveness (atomic timestamps per subscriber, 2-min staleness threshold — catches "alive but delivering zero messages" failure mode); **graceful HTTP shutdown** via `*http.Server` + `srv.Shutdown(10s)` drains active WS connections on SIGTERM instead of abrupt kill; **pong/read deadline** (`SetReadDeadline(pongWait)` + `SetPongHandler`) detects dead clients within 60s — no more leaked goroutines from unresponsive connections; **panic recovery** (`defer safego.RecoverPanic(...)`) on every independent goroutine (`RunWritePump`, `RunSubscriber`, `RunUserOfflineSubscriber`, `RunUserOnlineSubscriber`, `RunLastSeenDebounceWorker`) via shared `internal/safego` package — a single goroutine panic no longer crashes the process; **`sync.Once`-guarded `Conn.Close()`** — connection teardown channel wrapped with `sync.Once` + read-only `Conn.Done()` accessor, eliminating double-close panics under concurrent teardown races; **shared `*http.Client`** with tuned `Transport` on `Hub` struct and `HandlePresence` closure — no per-request/per-iteration client allocation (resource leak fix). See **`docs/adr/ARCHITECTURE_DECISIONS_CHAT_WS.md`** §8.
- **Infrastructure refactor (senior-grade):** backend Dockerfile restructured to true multi-stage (builder/runtime separation — production images ship without gcc/build tools, ~200–400MB smaller). All containers run as **non-root** users (`appuser`, `node`, `nginx`). `docker-compose.yml` is now production-baseline (`target: production`); dev overrides via `docker-compose.override.yml` (auto-loaded locally). Secrets isolation: **chat-ws** receives only `chat-ws/.env` — no access to backend secrets. Frontend switched to `nginxinc/nginx-unprivileged:alpine` (port 8080). Deprecated `main_worker.py` and compat `outbox-worker` service removed. **Production DEBUG guard:** `main.py` raises `RuntimeError` at import if `ENVIRONMENT=production` + `DEBUG=True`.
- **Container resource limits:** all 16 services in `docker-compose.yml` are constrained via reusable YAML extension-field profiles (`x-resources-micro/light/medium/redis`) providing `mem_limit`, `mem_reservation`, `memswap_limit` (swap disabled), `cpus`, and `pids_limit`. Budget sized for t3.medium (steady-state 3264MB / ~3500MB usable). Single source of truth — change one profile tier, all services using it update.
- **Container log rotation:** all 16 Compose services use a shared `x-default-logging` YAML anchor (`json-file` driver, `max-size: 10m`, `max-file: 3`) — caps each container at 30MB of log files, preventing disk exhaustion on long-running EC2 instances.
- **Monitoring port hardening:** Prometheus (`9090`) and Grafana (`3002`) ports in `docker-compose.yml` are bound to `127.0.0.1` — not reachable from the internet, accessible only via SSH tunnel or from the EC2 host itself.
- **Frontend internal nginx security headers:** the frontend container's internal nginx (`frontend/nginx.conf`) now returns `X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`, and `Permissions-Policy` headers at the `server` level — defense-in-depth even when the outer Compose nginx is bypassed.
- **Database check constraints (migration 024):** `CHECK` constraints enforce business invariants at the PostgreSQL level: `available_seats >= 1`, `price >= 0`, `distance_km >= 0`, `duration_min >= 0` on `rides`; `num_seats >= 1` on `bookings`. Prevents invalid data regardless of application-layer bugs.
- **Google OAuth phone collision fix (M8, migration 025):** `users.phone_number` changed to **nullable** with a **partial unique index** (`WHERE phone_number IS NOT NULL`). Google OAuth auto-signup no longer fabricates a placeholder phone from the Google `sub` — new OAuth users get `phone_number=NULL` until they explicitly add one. Eliminates `IntegrityError` collisions and stops polluting business data with fake phone numbers.
- **Backend CSP header (M4):** `SecurityHeadersMiddleware` now adds `Content-Security-Policy` to all API responses — strictest possible policy (`default-src 'none'`) for JSON endpoints, relaxed policy for Swagger UI (`/docs`, `/redoc`). Defense-in-depth: if a browser renders an API response directly, no scripts/frames can execute. Three-layer CSP architecture (edge nginx / backend middleware / frontend nginx) — [`docs/SECURITY_HEADERS.md`](docs/SECURITY_HEADERS.md).
- **Notification retry error classification (M3):** `NotificationManager._safe_send` no longer silently swallows all provider errors. Providers now raise **`TransientNotificationError`** (triggers RabbitMQ broker retry) or **`PermanentNotificationError`** (logged and swallowed). Push: `UnregisteredError`/`SenderIdMismatchError` = permanent (token cleared), network errors = transient. Email: `ValueError`/circuit-open = permanent, Brevo API/network = transient. Redis-based **idempotency guard** (24h TTL) prevents duplicate notifications on retry.
- **Pagination max enforcement (audit-verified):** every paginated endpoint across the entire API enforces `Query(le=...)` — public endpoints capped at 50–100, admin endpoints at 200–500. Constants defined once in router modules (`_QUERY_MAX_*`).
...
- Atomic Redis rate limiting with split algorithms (sliding-window for auth and ride creation, token-bucket for chat) + `X-RateLimit-*` headers.
...
- Stage 3d (safe subset) completed for Chat: `useChatUnreadMessages` and `useChatNotificationsFeed` moved from manual intervals to React Query polling, and `Messages` uses **`useInfiniteQuery`** + **`GET /chat/conversations`** cursor pagination (`qk.chat.conversations(limit)`) while preserving WS-driven chat transport layers unchanged.
- **Passenger match cap + notification feed cursor + cancel-ride DB batching:** immediate ride matches on create/refresh are capped at **20** (`find_rides_by_coordinates`); in-app **`/users/me/notifications`** now uses cursor pagination (`limit`, `after` → `items`, `next_cursor`, `has_more`) with stable keyset order `created_at DESC, booking_id DESC` across driver+passenger sources; driver ride cancel recomputes `passenger_requests.status` via one booking query + bulk updates; **Groups** list uses **`get_member_counts_batch`** instead of per-group count queries — see **[docs/FEATURE_DECISIONS.md#api-read-caps-batch-status](docs/FEATURE_DECISIONS.md#api-read-caps-batch-status)** and **[docs/architecture/API.md](docs/architecture/API.md)**.
- **Passenger cancel request — bulk bookings + enum alignment:** **`DELETE …/passengers/{request_id}/cancel`** uses **`CRUDBooking.bulk_cancel_bookings_for_request`** (aggregate seats per ride for seat-holding statuses, **`FOR UPDATE`** on affected **`rides`**, single bulk **`UPDATE bookings`**), instead of an per-booking loop. Alembic **`019_booking_lifecycle_enum`** adds **`en_route`**, **`arrived`**, **`trip_in_progress`** to PostgreSQL **`booking_status`** so ORM `IN (...)` filters match Python **`BookingStatus`**. Integration tests in **`backend/tests/test_bookings.py`** — **[docs/architecture/DATABASE.md](docs/architecture/DATABASE.md)** (migrations + race table), **[docs/FEATURE_DECISIONS.md#api-read-caps-batch-status](docs/FEATURE_DECISIONS.md#api-read-caps-batch-status)**.
...
- Stage 5 cleanup completed: `useMyRequests` migrated to React Query (`qk.passengers.requests` + mutation cache patching for cancel/expire), and `AuthContext` initial boot effect fixed with a proper `AbortController`-based cancellation (removed dead `cancelled` boolean guard).
- **M2 — AbortController request cancellation:** all hand-written API functions (`chat.ts`, `groups.ts`, `users.ts`, `geo.ts`, `presence.ts`, `bookings.ts`) accept an optional `{ signal?: AbortSignal }` parameter forwarded to Axios; reusable `useAbortSignal()` hook (`src/hooks/useAbortSignal.ts`) manages controller lifecycle; four manual-fetch hooks migrated to React Query (`useGroupManageLists` members/rides, `JoinGroup`, `useMessageThread` partner presence, `RouteMapModal` maps key); remaining `useState`-based hooks (`useConversationMessages`, `useChatPopup`, `AuthContext`, `useLocationBroadcast`, `useProfile` avatar polling) wired with `AbortController` directly; `axios.isCancel(err)` guards added at every catch site.
- OpenAPI-to-frontend contract hardening shipped: dedicated [`openapi-contract.yml`](.github/workflows/openapi-contract.yml) workflow exports `app.openapi()` from FastAPI on every run via [`backend/scripts/export_openapi.py`](backend/scripts/export_openapi.py), runs Orval, and fails on drift in `frontend/src/api/generated/`. The snapshot (`frontend/openapi-snapshot.json`) is a gitignored build artifact — single source of truth is FastAPI. Local DX: `make openapi` (root) or `npm run openapi:sync` (frontend). Workflow runs without Postgres — `app.openapi()` is lazy.
...
- **ConfirmModal aria fix (M12):** removed erroneous `aria-hidden="true"` from backdrop wrapper that hid the entire dialog from screen readers; `aria-modal="true"` on the inner panel already handles background-content semantics correctly.
...
- **H20 — Google Sign-In XSS-safe React fallback:** imperative `innerHTML`/`createElement` fallback button replaced with React state + `<GoogleFallbackButton />` component; triple try/catch flattened; timeout detection unified via shared `isTimeoutOrAbortError` util; error strings migrated to i18n (`auth:error_origin_not_allowed`, `auth:backend_timeout`).
...
- **Stage 2 hardening:** open redirect protection on post-login `?from=` navigation (both Login and Google Sign-In); `model_validator(mode="after")` enforces `SECRET_KEY` **non-empty in all environments** (crash at startup if empty) and **≥ 32 chars** in production; Outbox `MAX_RETRIES = 5` caps permanently failing events as `FAILED`; logging hygiene — removed stale `print()` calls from outbox repository, auth router, and calendar exporter.
...
- i18n dead-key audit & cleanup: scanned ~430 translation keys across 8 namespaces against all frontend source files; removed 57 unused keys (~13%) from both `he` and `en` locales in `src/i18n/locales/` and `public/locales/` — superseded keys (`resendCode` → `resendNewCode`, `noBookingsPassenger` → `noPassengerBookings`, `driverTab` → `iAmDriver`, etc.), unreferenced error keys, and leftover UI labels. Fixed runtime bug: 2 auth keys (`backend_timeout`, `error_origin_not_allowed`) were missing from the HTTP-loaded `public/locales/auth.json` despite being referenced in code.
...
- **H19 — Refresh token moved to HttpOnly cookie:** refresh token is no longer returned in JSON or stored in localStorage. Backend sets a **`Secure; HttpOnly; SameSite=lax`** cookie (`linkup_refresh_token`) scoped to `/api/v1/auth` on login, Google sign-in, and refresh; the frontend sends `withCredentials: true` (cookie sent automatically). `POST /auth/refresh` reads the cookie instead of a request body. `POST /auth/logout` clears the cookie. One-time cleanup removes the legacy `linkup_refresh_token` localStorage key. Eliminates XSS-based token theft while keeping CSRF risk minimal (SameSite + path scope + no state-changing GET).
- Frontend sourcemap upload hardening shipped: Vite integrates `@sentry/vite-plugin` behind production+env guards (`SENTRY_AUTH_TOKEN`, `SENTRY_ORG`, `SENTRY_PROJECT`), CI injects secrets only in `publish-image`, uploaded sourcemaps are deleted from `dist` after upload, and Dockerfile uses `ARG`-only (no `ENV`) so Sentry token is not persisted in builder layer metadata.
- **Credential hygiene:** `alembic.ini` `sqlalchemy.url` placeholder replaced with a comment — the real URL is injected at runtime by `env.py` via `settings.DATABASE_URL`; no hardcoded credentials in committed config files.
...
- Edge hardening in nginx: **`listen 443 ssl`** (HTTP/1.1 over TLS after rendering `nginx/nginx.conf` from `nginx/nginx.conf.template`; add `http2` to the directive if you want ALPN HTTP/2 — see **`docs/SECURITY_HEADERS.md`**), HSTS (`includeSubDomains`), `X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`, `Permissions-Policy`, enforcing **`Content-Security-Policy`** — **`script-src`** בלי **`'unsafe-inline'`** (בוטסטרפ שפה/ערכת נושא ב־**`frontend/public/bootstrap.js`** → **`/bootstrap.js`** לפני **`/config.js`**); allowlists ל־Stripe frames, Google Sign-In, `connect-src` (Firebase/Sentry/analytics/uploads/Google GIS); **`report-uri`** מ־**`SENTRY_REPORT_URI`** (אחרי render) — **`docs/SECURITY_HEADERS.md`**.
...
|----------|------------------|------|
| backend  | Python (FastAPI) | REST API, auth, rides, bookings, chat CRUD, **groups**, passengers (requests/matches), **admin JSON API** (`/api/v1/admin/*`), events via Outbox; worker wake uses LISTEN/NOTIFY on a **direct** Postgres DSN (`DATABASE_URL_DIRECT`) while app traffic uses PgBouncer |
...
| chat-ws  | Go 1.23          | WebSocket server; chat + typing + presence; Redis Pub/Sub including **`user:online` / `user:offline`** and **`user:*:events`** (ride/maintenance-style JSON to the logged-in client). **Reconnect-safe:** all subscribers use exponential backoff (1s→30s) with **automatic reset after stable connections**; connection lifecycle uses `sync.Once`-guarded `Conn.Close()` + read-only `Conn.Done()` channel — double-close panics are structurally impossible. **Resource-safe:** shared `*http.Client` with tuned `Transport` on `Hub` struct and `HandlePresence` closure — no per-request/per-iteration client allocation. **Operationally hardened:** `/healthz` with subscriber liveness tracking, graceful `http.Server.Shutdown`, pong/read deadlines for dead client detection, `defer safego.RecoverPanic` on all goroutines, and **origin validation** (pre-computed allowlist map, empty-origin rejection, case-insensitive normalization, structured rejection logging). |
...
- ✅ **Rides & bookings:** ride search, booking requests, driver approve/reject; **start/end ride** from "My Bookings" (driver tab; requires at least one confirmed passenger). **My Bookings** uses **aggregated REST reads** with a **split active vs history**: [`GET /bookings/driver-summary/active`](docs/architecture/API.md) + [`GET /bookings/driver-summary/history`](docs/architecture/API.md) (cursor `after` / `next_cursor`), and the same pattern for **passenger** — so the web app avoids N+1 and keeps history paginated. Frontend: `useQuery` for active + `useInfiniteQuery` for history ([`useMyBookingsDriver.ts`](frontend/src/pages/MyBookings/useMyBookingsDriver.ts), [`useMyBookingsPassenger.ts`](frontend/src/pages/MyBookings/useMyBookingsPassenger.ts), [`useMyBookings.ts`](frontend/src/pages/MyBookings/useMyBookings.ts)); mapping in [`myBookings.mappers.ts`](frontend/src/pages/MyBookings/myBookings.mappers.ts). Backend: [`BookingReadsService`](backend/app/domain/bookings/booking_reads_service.py) + shared cursor core [`backend/app/core/pagination/cursor.py`](backend/app/core/pagination/cursor.py); soft cap **200** rows for active lists — see [`DATABASE.md`](docs/architecture/DATABASE.md).
...
- ✅ **Missed messages + read receipts:** reconnect backfill now uses a dedicated gap endpoint (`/messages/gap?since_message_id=`) and loops only when the server reports `truncated=true` (with `last_message_id` as the next anchor). History pagination stays opaque-keyset on `/messages?after=`. Read receipts continue to rely on `conversation_participants.last_read_message_id` and color `✓✓` on outgoing messages covered by the partner's read cursor. **Scroll behavior** is intent-based: loading older history preserves scroll position, gap-fill does not scroll, and new inbound messages scroll to bottom only if the user is already near the bottom.
...
- ✅ **Chat detail / booking-chat redundant query elimination + pending bookings eager loading:** `_get_partner_read_info` ([`chat/service.py`](backend/app/domain/chat/service.py)) replaced two helpers that each re-fetched the conversation; single query with `partner_id` already in scope → 5–7 queries down to 2–4 per endpoint. `get_ride_bookings_by_status_with_relations` ([`bookings/crud.py`](backend/app/domain/bookings/crud.py)) adds `joinedload` for passenger relationships so `GET /bookings/ride/{id}/pending` returns `passenger_name` instead of `null`. See [FEATURE_DECISIONS.md — chat detail](docs/FEATURE_DECISIONS.md#chat-detail-redundant-refetch).
...
- ✅ **Driver / group ride lists:** **`GET /api/v1/rides/me`** and **`GET /api/v1/groups/{id}/rides`** use **cursor pagination** (`limit`, `after` → `rides`, `next_cursor`, `has_more`) with keyset order `departure_time DESC, ride_id DESC`; composite indexes **`018`** support this query path. See **`docs/architecture/API.md`**.
- ✅ **PgBouncer (transaction pooling, internal-only):** backend + workers connect via `pgbouncer` service (no public port), `migrate` stays direct to `db`, asyncpg statement cache is disabled at engine connect for compatibility, and PgBouncer runtime now uses a custom image (`infrastructure/pgbouncer/Dockerfile`) so mounted config is not overridden by third-party entrypoints. **Outbox** `LISTEN`/`NOTIFY` in `notification-worker` uses **`DATABASE_URL_DIRECT`** (`POSTGRES_HOST_DIRECT` / `POSTGRES_PORT_DIRECT`, default `db`/`5432`) because transaction pooling does not forward `NOTIFY` to pooled clients.
...
- ✅ **Redis reconnect hardening:** reconnect loop uses retry with exponential backoff (1s→30s cap) for resilient long-lived pub/sub connections, with automatic backoff reset after stable connections to prevent permanent 30s reconnection delay.
...
- ✅ **Passenger requests (בקשות טרמפ):** חיפוש נסיעות (`GET …/passengers/search-rides`) בלי שמירה — אופציונלי **`departure_date`** / **`departure_time`** / **`departure_time_to`** (פירוט ב־**`docs/architecture/API.md`**); **שמירת התראה** — `POST …/passengers/` עם אותם פרמטרי מסלול + `is_notification_active` / `group_id`; הצטרפות מתוצאות (`request-ride-from-search`); ביטול בקשה; התאמות; קישור אופציונלי מבוקינג לבקשה
...
- ✅ **In-app notification feed (web):** מקור הרשימה הוא **`GET /api/v1/users/me/notifications`** ב-cursor pagination (`limit`, `after`, `next_cursor`, `has_more`, **`unread_count`**); כל פריט כולל **`is_read`** מ-DB (טבלת `notification_reads`, מיגרציה **023**). סימון קריאה: **`PATCH .../read`** (פריטים ספציפיים) ו-**`PATCH .../read-all`** (batch). מסך ההתראות משתמש ב-`useInfiniteQuery`, ובאדג'ים ב-`ChatContext` ממשיכים pull קל של העמוד הראשון (`limit=20`) ב-React Query. רענון חי מגיע מאירועי `user:{id}:events` על חיבור chat-ws (invalidate `notifications`), עם polling fallback כל ~5 דקות ב-`useChatNotificationsFeed`. מחליף localStorage צד-לקוח שצבר נתונים ללא הגבלה — מאפשר סנכרון בין מכשירים. פירוט: [`docs/architecture/API.md`](docs/architecture/API.md), [`docs/architecture/REALTIME.md`](docs/architecture/REALTIME.md), [`docs/architecture/NOTIFICATIONS.md`](docs/architecture/NOTIFICATIONS.md).
- ✅ **Chat RQ migration (safe subset):** manual polling/fetch surfaces were migrated to React Query — [`useChatUnreadMessages`](frontend/src/context/useChatUnreadMessages.ts) now uses `qk.chat.unread()` polling + invalidate refresh API, [`useChatNotificationsFeed`](frontend/src/context/useChatNotificationsFeed.ts) now uses `qk.notifications.all()` polling + invalidate refresh API, and [`Messages.tsx`](frontend/src/pages/Messages.tsx) uses **`useInfiniteQuery`** (`qk.chat.conversations(limit)`, infinite scroll sentinel); WS transport/message-stream layers remain unchanged by design.
- ✅ Google OAuth and email/password auth with JWT + **HttpOnly cookie refresh token** (H19: refresh token moved from localStorage to a `Secure; HttpOnly; SameSite=lax` cookie scoped to `/api/v1/auth` — eliminates XSS token theft; frontend uses `withCredentials: true`). **M8:** Google OAuth auto-signup creates users with `phone_number=NULL` (no more placeholder phones); partial unique index preserves phone uniqueness for email/password registrations.
- ✅ Geo: distance, route display, PostGIS-backed queries; **ride preview cache** (Redis, 24h) for full preview payload between steps + **geocode cache** (`geocode_cache.get_coordinates`, stampede-safe) used by **`get_full_routing_data`** (`domain/geo/processor.py`) so address→coords for preview hits Redis before Google Geocoding; Directions unchanged
- ✅ **Passenger/driver read caps:** **`GET …/passenger/passengers/me`** — **cursor pagination** (`items`, `next_cursor`, `has_more`; `limit` bounded by `PASSENGER_REQUESTS_*` constants); פרונט: **`fetchMyPassengerRequests`** / **`useMyRequests`** עם `useInfiniteQuery` (`cursor`, `limit=100`). **`GET …/users/me/notifications`** — cursor-based (`limit` default 20, max 100; `after`, `next_cursor`, `has_more`) במקום hard-cap לא מתועד. **`GET …/bookings/ride/{ride_id}/manifest`** — עד **100** שורות ב-**SELECT** אחד, מיון **confirmed** לפני **pending**, שקיפות בסט טוטלים (**`confirmed_total`**, **`pending_total`**, **`manifest_truncated`**) ו-**`total_confirmed_passengers`** כספירת מאושרים ב-DB
- ✅ **GPS live tracking:** driver and passengers share location during active rides (REST → Redis Pub/Sub → WebSocket). Permission and ride-status checks for **POST /bookings/{id}/location** and **passenger-location** live in [`BookingLocationService.broadcast_driver_location` / `broadcast_passenger_location`](backend/app/domain/bookings/location_service.py) (thin routers; classes also re-exported from [`service.py`](backend/app/domain/bookings/service.py) for compatibility). The web app resolves the driver’s `booking_id` for GPS from the **confirmed passenger row** on the active ride (same data as the **`/bookings/driver-summary/active`** passenger list). Location POSTs are throttled (~1.5s); map markers update in place — see [`docs/architecture/REALTIME.md`](docs/architecture/REALTIME.md) (GPS Tracking).
...
- ✅ Outbox pattern for reliable event dispatch (RabbitMQ + Redis chat delivery)
...
- ✅ **i18n, לוקאליזציה וטיפוגרפיה (ווב):** **i18next** עם משאבים ב־`frontend/src/i18n/locales/{he,en}/` (למשל `common.json`, `nav.json`, `rides.json`, `admin.json`). **`LangContext`** מגדיר `dir` על `<html>` ומעדכן **`--font-primary`** (עברית: Heebo, אנגלית: DM Sans). פורמט תאריכים/שעות לפי שפת הממשק דרך **`frontend/src/utils/date.ts`** ו־**`getLocale()`** — בלי `he-IL` קשיח ברוב הזרימות. ב־hooks ובלוגיקה מחוץ ל־React, fallback לטקסטי שגיאת API אחרי **`getApiErrorMessage`** עובר דרך **`apiErr('err_*')`** ב־[`frontend/src/utils/i18nError.ts`](frontend/src/utils/i18nError.ts) (מפתחות **`common:err_*`**); שגיאות ולידציה צד-לקוח (geolocation, מוצא/יעד, מסלול) דרך **`t('err_*')`** מ-namespace פיצ'רי (`rides:err_*`); UI copy ב-`Messages.tsx` דרך **`common:msg_*`** — אפס מחרוזות עברית קשיחות ב-hooks ובדפים. **מסכי אדמין** (`/admin/*`) — namespace **`admin`** (bundled, ~100 מפתחות) עם `useTranslation('admin')` בכל דף. **CSS Modules:** `font-family: var(--font-primary)` / `var(--font-numeric)`; חריג מכוון לכפתור שפה — **`LangToggle`** (מונוספייס). פירוט החלטות: **[docs/adr/ARCHITECTURE_DECISIONS_FRONTEND.md](docs/adr/ARCHITECTURE_DECISIONS_FRONTEND.md)** (סעיפים 10–12).
- ✅ **S.7 Asset hardening (ווב):** lazy/eager loading הוגדר נקודתית ל-`img` קריטיים מול רשימות, נוספו hints ב-`index.html` (`preconnect` ל-`linkup-uploads.s3.amazonaws.com` + locale preloads), ו-`i18n` הועבר ל-hybrid loading: `common`/`nav`/`admin` נשארים bundled inline, בעוד `auth`/`rides`/`bookings`/`groups`/`profile`/`billing` נטענים עצלנית דרך `i18next-http-backend` מ-`/locales/{{lng}}/{{ns}}.json` עם קבצי runtime תחת `frontend/public/locales/`.
...
| `nginx/`   | קונפיג Nginx ל־Compose — reverse proxy ל־API/chat-ws/frontend, TLS ב־443 (**`listen 443 ssl`** — HTTP/1.1 אלא אם מוסיפים `http2`), **gzip compression** (`gzip_proxied any`, text/json/css/js/xml/svg), **API rate limiting** (`limit_req_zone` 30r/s per IP on `/api/v1/`, burst=50, 429 status), security headers ו־**CSP מאוכף** + `report-uri` (מתועד ב־[`docs/SECURITY_HEADERS.md`](docs/SECURITY_HEADERS.md)) |
| `mobile/`  | React Native (Expo) app |
| `db/`      | Reference schema (`schema.sql`) and utility scripts; migrations live in `backend/alembic/` |
| `infrastructure/backup/` | Database backup pipeline: Dockerfile (postgres:15 + AWS CLI), `backup.sh` (pg_dump → encrypt → S3), `restore.sh` (S3 → decrypt → pg_restore), S3 lifecycle rules |
...
- **Worker healthchecks (M10):** all three async workers (`notification-worker`, `task-worker`, `ai-worker`) now have Docker healthchecks using PID-based liveness (`python -c "import os; os.kill(1, 0)"`) — detects main process crash independently of container exit status; interval 30s, retries 3, start_period 30s.
- **RabbitMQ retry & DLQ:** notifications and avatar queues use broker-native retry via `retry_exchange` + `<queue>.retry` TTL queue + `x-death` counting (no manual republish loop in workers). After max retries, messages are routed to per-queue `.dlq`. **M3:** notification providers now classify errors as `TransientNotificationError` (triggers broker retry via nack) or `PermanentNotificationError` (swallowed after logging); Redis-based dedup guard (24h TTL) prevents duplicate sends on retry. See `docs/architecture/EVENTS.md`.
...
- **Database backups (automated):** daily pg_dump → AES-256 encrypt → S3 (cron 03:00 UTC); pre-deploy backup triggered by `deploy-ec2.yml` before image pulls; restore scripts and backup freshness monitoring via `scripts/ops/check-backup-health.sh`. Full runbook: [`docs/BACKUP_AND_RECOVERY.md`](docs/BACKUP_AND_RECOVERY.md).
- **Pessimistic locking:** booking approve/cancel use `SELECT ... FOR UPDATE` on the ride to avoid race conditions.
- **Connection pooling:** async SQLAlchemy pool — `pool_size`, `max_overflow`, `pool_timeout`, `pool_recycle`, `pool_pre_ping` מ-`settings` / `.env` (`DB_POOL_*`). **`statement_timeout`** הוא **דו-שכבתי**: ערך אופרטיבי ברמת session דרך `connect_args.server_settings` ב-`app/db/session.py` (מ-**`DB_STATEMENT_TIMEOUT_MS`**, ברירת מחדל 30s), וגם ceiling דיפנסיבי קשיח של 60s ברמת role דרך migration **017**. אינדקסים על rides/bookings/group_members/passenger_requests/chat message access (כולל `bookings.request_id`, `messages.sender_id`; **022**: composite `idx_bookings_passenger_status`, `idx_bookings_ride_status`, `idx_passenger_requests_passenger_status`, partial `idx_outbox_events_status_created`, `idx_users_last_active_at`) — see `docs/architecture/DATABASE.md`.
...
- **Container resource limits:** every Compose service has hard memory limits (`mem_limit`), soft reservations (`mem_reservation`), swap disabled (`memswap_limit` = `mem_limit`), CPU caps (`cpus`), and fork-bomb protection (`pids_limit`) via tiered YAML extension profiles. Prevents a single misbehaving container from OOM-killing the host.
- **PgBouncer secrets flow (deploy-safe):** `userlist.txt` is **not** generated on host/CI anymore. The PgBouncer container now renders `/var/lib/pgbouncer/userlist.txt` at startup from env vars (`POSTGRES_USER`, `POSTGRES_PASSWORD`, `PGBOUNCER_ADMIN_PASSWORD`) via `entrypoint.sh`, removing host bind-mount UID coupling. **Edge CSP:** deploy still renders **`nginx/nginx.conf`** from **`nginx/nginx.conf.template`** using **`SENTRY_REPORT_URI`** from **`backend/.env`**; locally run **`bash scripts/ops/render-nginx-conf.sh`** before `docker compose --env-file backend/.env --env-file frontend/.env --profile prod up -d` (או מקביל עם `make` + פרופיל).
- **Auth hardening:** bcrypt hashing/verify רץ ב-**thread pool** (`asyncio.run_in_executor`) כדי לא לחסום את event loop; **rate limit** על `/register` ועל login/refresh (Redis), בצ'אט על `POST /chat/conversations/{conversation_id}/messages` פר-משתמש (30 הודעות/דקה, fail-open אם Redis לא זמין), ובנסיעות על `POST /rides/` ו-`POST /rides/preview-routes` (10 לשעה פר-משתמש, Sliding Window); OTP: `secrets`, `hmac.compare_digest`, מונה ניסיונות + איפוס בקוד חדש; **מניעת username enumeration בלוגין** (אותה תגובת שגיאה לאימייל לא קיים ולסיסמה שגויה — OWASP); **refresh token in HttpOnly cookie** (H19 — XSS-safe, SameSite=lax, path-scoped); **M4 — refresh token hashed at rest** (SHA-256 in DB, constant-time compare via `hmac.compare_digest`, **schema-derived allowlist** in `CRUDUser.update` — only fields declared in `UserUpdate` Pydantic schema are writable; `UserUpdate` is a standalone `BaseModel` with `extra="forbid"` (Pydantic rejects unknown fields), same for `GroupUpdate`/`RideUpdate`; privileged fields like `is_admin`/`is_premium`/`is_active`/`is_verified` require dedicated CRUD methods (`mark_as_verified`), migration **021**) — ראו `docs/ENGINEERING_HIGHLIGHTS.md` ו-`ARCHITECTURE.md` (Security).
...
- **Single-EC2 rolling CD (senior pragmatic).** Instead of full blue/green infra, production deploy runs on the same host via **[`deploy-ec2.yml`](.github/workflows/deploy-ec2.yml)** (`workflow_run` after green **Backend / Frontend / Chat-WS / Email renderer** CI on `main`): prefer immutable GHCR tag (`sha`, with fallback to `latest` if missing), internal smokes for `frontend` and `email-renderer` before the backend gate, then post-deploy checks for backend readiness (internal **`/readyz`** from **`linkup_backend`**) and Firebase env presence; rollback to the previous tag, with pull fallback to `backend:latest` when the old digest is gone. **Deploy-time env hardening:** syncs `REDIS_PASSWORD` → `chat-ws/.env`, injects `POSTGRES_HOST_DIRECT`/`POSTGRES_PORT_DIRECT` if absent, strips stale `REDIS_SENTINEL_HOST`, deduplicates `PGBOUNCER_ADMIN_PASSWORD`, and runs `--remove-orphans` to clean leftover containers from removed services. This keeps ops robust on `t3.medium` without extra AWS cost.
...
- **Outbox pattern.** Notifications (email, push), chat realtime delivery, and other side effects are triggered by domain events. Publishing directly to external infra in the same transaction as the DB write risks event loss on crash or broker/network failure. Writing the event to `outbox_events` in the same transaction, then letting **`notification-worker`** run **`run_outbox_worker`** (LISTEN/NOTIFY on **`DATABASE_URL_DIRECT`** — direct Postgres, bypassing PgBouncer — plus fallback polling) dispatch to RabbitMQ/Redis keeps “at-least-once” delivery and decouples API latency from downstream availability.
...
**Supply chain hardening:** all action references across 6 workflow files are **pinned to immutable commit SHAs** (`uses: owner/action@<full-sha> # vX`) — prevents execution of unreviewed code if a tag is moved to a malicious commit. Dependabot `github-actions` ecosystem keeps SHAs current via automated PRs.

| Service   | Workflow | Steps |
|-----------|----------|-------|
| backend   | `backend-ci.yml`  | lint (Ruff), format check, migrations (`uv run alembic upgrade head` on ephemeral `test_db`), `scripts/ops/check-migration-head.sh`, targeted RabbitMQ pytest, full `uv run pytest tests/`, Docker build → push to GHCR (`latest` + `sha`). **GHA Docker layer cache** (`setup-buildx-action` + `cache-from/to: type=gha,mode=max`) on all 4 build targets. |
| chat-ws   | `chat-ws-ci.yml`  | build (Go 1.23), vet, Docker build → push to GHCR. Concurrency group + timeout. **GHA Docker layer cache.** |
| frontend  | `frontend-ci.yml` | `quality` (ESLint, **Vitest**, build, bundle-size), `publish-image` (main push only, GHCR). **GHA Docker layer cache.** Contract codegen moved to its own workflow. |
...
- **GitHub Actions:** weekly — keeps SHA-pinned action references up to date with the latest versions (labels: `dependencies`, `ci`).
...
- Google Sign-In fallback button (H20) uses React state + component rendering — zero `innerHTML` usage in the auth flow.
