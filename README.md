...
- Edge hardening in nginx: **`listen 443 ssl`** (HTTP/1.1 over TLS after rendering `nginx/nginx.conf` from `nginx/nginx.conf.template`; add `http2` to the directive if you want ALPN HTTP/2 — see **`docs/SECURITY_HEADERS.md`**), HSTS (`includeSubDomains`), `X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`, `Permissions-Policy`, enforcing **`Content-Security-Policy`** — **`script-src`** בלי **`'unsafe-inline'`** (בוטסטרפ שפה/ערכת נושא ב־**`frontend/public/bootstrap.js`** → **`/bootstrap.js`** לפני **`/config.js`**); allowlists ל־Stripe frames, Google Sign-In, `connect-src` (Firebase/Sentry/analytics/uploads); **`report-uri`** מ־**`SENTRY_REPORT_URI`** (אחרי render) — **`docs/SECURITY_HEADERS.md`**.
