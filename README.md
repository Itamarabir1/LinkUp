...
- **חיפוש נסיעות נוסע (`GET …/search-rides`):** פרמטרים **`departure_date`** (יום מלא **Asia/Jerusalem**), **`departure_time`** (חלון **±2 שעות**), או **`departure_time`** + **`departure_time_to`** (טווח כולל); **הדדיות** → **422**. **פרונט:** **`buildManualRideSearchParams`** לפי **`searchMode`** (**`date_only` / `datetime` / `time_range`**) + **`buildParamsFromAiResult`** ללא `departure_time` מזויף ב־תאריך־בלבד; Orval מ־**`openapi-snapshot.json`**; בדיקות: **`backend/tests/domain/test_ride_search_request.py`**, **`frontend/.../rideSearchParams.test.ts`** — פירוט **`docs/architecture/API.md`**.
