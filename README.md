...
- **Stage 2 hardening:** open redirect protection on post-login `?from=` navigation (both Login and Google Sign-In); `model_validator(mode="after")` enforces `SECRET_KEY ≥ 32 chars` in production; Outbox `MAX_RETRIES = 5` caps permanently failing events as `FAILED`; logging hygiene — removed stale `print()` calls from outbox repository and auth router.
