...
- **chat-ws operational hardening (H7–H10):** **`/healthz`** endpoint verifies Redis connectivity **and** subscriber goroutine liveness (atomic timestamps per subscriber, 2-min staleness threshold — catches "alive but delivering zero messages" failure mode); **graceful HTTP shutdown** via `*http.Server` + `srv.Shutdown(10s)` drains active WS connections on SIGTERM instead of abrupt kill; **pong/read deadline** (`SetReadDeadline(pongWait)` + `SetPongHandler`) detects dead clients within 60s — no more leaked goroutines from unresponsive connections; **panic recovery** (`defer safego.RecoverPanic(...)`) on every independent goroutine (`RunWritePump`, `RunSubscriber`, `RunUserOfflineSubscriber`, `RunUserOnlineSubscriber`, `RunLastSeenDebounceWorker`) via shared `internal/safego` package — a single goroutine panic no longer crashes the process. See **`docs/adr/ARCHITECTURE_DECISIONS_CHAT_WS.md`** §8.
...
- **Stage 2 hardening:** open redirect protection on post-login `?from=` navigation (both Login and Google Sign-In); `model_validator(mode="after")` enforces `SECRET_KEY ≥ 32 chars` in production; Outbox `MAX_RETRIES = 5` caps permanently failing events as `FAILED`; logging hygiene — removed stale `print()` calls from outbox repository, auth router, and calendar exporter.
...
- ✅ **In-app notification feed (web):** מקור הרשימה הוא **`GET /api/v1/users/me/notifications`** ב-cursor pagination (`limit`, `after`, `next_cursor`, `has_more`); מסך ההתראות משתמש ב-`useInfiniteQuery`, ובאדג'ים ב-`ChatContext` ממשיכים pull קל של העמוד הראשון (`limit=20`) ב-React Query. רענון חי מגיע מאירועי `user:{id}:events` על חיבור chat-ws (invalidate `notifications`), עם polling fallback כל ~5 דקות ב-`useChatNotificationsFeed`. פירוט: [`docs/architecture/API.md`](docs/architecture/API.md), [`docs/architecture/REALTIME.md`](docs/architecture/REALTIME.md).
...
- ✅ Google OAuth and email/password auth with JWT + **HttpOnly cookie refresh token** (H19: refresh token moved from localStorage to a `Secure; HttpOnly; SameSite=lax` cookie scoped to `/api/v1/auth` — eliminates XSS token theft; frontend uses `withCredentials: true`)
