...
- **Realtime badges (chat + in-app):** כל הפריימים ל־נמען עוברים ב־Redis על **`user:{id}:events`** — **`invalidate`** עם **`unread_messages`** (אחרי שליחת הודעה + `count`) או **`notifications`** (אחרי outbox **`WebSocketProvider`**), בנוסף ל־**`UserEvent`** מ־**`publish_user_event`**. **chat-ws** לא מנוי יותר ל־**`chat:notification:*`**. בפרונט: **`useUserEventStream`** ב־**`ChatContext`**, **`setUnreadDirect`** למטמון React Query, ו־**`linkup:user-event`** לתאימות מסכים; **`useLayoutShell`** בלי WS — **`docs/architecture/REALTIME.md`**, **`chat-ws/ARCHITECTURE.md`**.
