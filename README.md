...
- **Outbox pattern.** Notifications (email, push), chat realtime delivery, and other side effects are triggered by domain events. Publishing directly to external infra in the same transaction as the DB write risks event loss on crash or broker/network failure. Writing the event to `outbox_events` in the same transaction, then letting **`notification-worker`** run **`run_outbox_worker`** (LISTEN/NOTIFY + fallback polling) dispatch to RabbitMQ/Redis keeps “at-least-once” delivery and decouples API latency from downstream availability.
